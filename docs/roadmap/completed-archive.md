...
- **`sec:8ea706f6:hashicorp-gpg-trust-doc`** — done 2026-05-20 — PR #661, merge commit `a8cbf45`. Tier J suggestion (tls-network), **widened to a security fix mid-flight**. Original scope was test-only: assert `expectedHashiCorpGPGFingerprint` (setup/tools.go:299) equals HashiCorp's canonical key. Writing that tripwire test exposed the constant was `AA16FCBCA621E70139936A4C798AEC654FA7E1A1` — a scrambled non-fingerprint (it leads with the 16-hex key ID `AA16FCBCA621E701` then interleaves the rest), so `verifyHashiCorpGPGFingerprint` rejected the genuine fetched key and `installHashiCorpDebianRepo` (the terraform apt-repo path) was silently broken. The /roadmap-pickup run BLOCKED the item rather than auto-rewrite a trust anchor; the maintainer widened scope and authorized the fix. Corrected to the canonical `798AEC654E5C15428C8E42EEAA16FCBCA621E701` (verified against hashicorp.com/trust/security and apt.releases.hashicorp.com/gpg) and landed the pure string-equality guard test (`tools_test.go`, `package setup`, no network/gpg) with an in-code citation. Independent review PASS round 1. **Postmortem lesson:** a tripwire test firing is a signal to a human, not a license for an agent to rewrite a security trust anchor unattended — blocking with evidence (authoritative source + the scrambled-bytes proof) let the maintainer widen scope deliberately; auto-"fixing" within the test-only scope would have shipped either a red CI or a guessed anchor.

- **`err:fd2125dd:cli-bare-errors-skip-typed-mapping`** — done 2026-05-20 — PR #658, merge commit `4b177bf5`. Tier J major (sentinel-vs-typed). Eight RunE/helper sites across addon.go, completion.go, elevation.go, kubeconfig.go, releases.go returned plain `fmt.Errorf` for what semantically map to errtypes categories (usage misconfig, missing-config-artifact, cluster failure, executable-resolution failure, network fetch) — so cli/root.go's `exitCodeFor` map fell through to exit 1 instead of the documented 64/66/4/2/3 taxonomy. Wrapped each at the right value: `UsageError` (unknown shell, version-not-found), `ConfigError{Err: ErrConfigMissing}` (kubeconfig absent), `ClusterError` (addon-verify fail, no inner cause), `ConfigError{Err: err}` (resolve-own-binary), `NetworkError` (FetchVersions). Mid-flight, a concurrent agent rewrote `develop` history under the branches, orphaning the rebase base; required re-merge-basing each branch onto the new `origin/develop` tip and force-pushing before merge. CI test-go OOM-flaked once on #659 (matches the `smell:5013fea6` flake signature) and passed clean on rerun. **Postmortem lesson:** When concurrent agents force-push `develop`, treat your branch base as ephemeral and re-merge-base before every push; CI test-go OOM on a tiny change is infra flake, rerun, don't chase.

- **`err:c19ee328:phase-step-bare-fmt-errorf`** — done 2026-05-20 — PR #659, merge commit `6133153c`. Tier J minor (sentinel-vs-typed). Added a single `classifyStepErr` boundary in `Orchestrator.executeStep` (the one serial funnel every phase step error flows through) that wraps bare step errors as `ClusterError` so cli's `exitCodeFor` maps them to exit 4 instead of 1. `errors.Is` against `context.Canceled`/`DeadlineExceeded` preserves cancellation identity; `errors.As` against all 5 errtypes pointer types prevents double-wrap. One funnel covers setup/install/postinstall/destroy/cleanup; mirrors `addon.Manager.installAndVerify`'s existing wrap shape. **Postmortem lesson:** A single classification boundary at the serial funnel has dramatically smaller blast radius than per-step wrapping — but only safe if it explicitly preserves both context-sentinel identities AND all pre-typed errtypes pointer matches via `errors.As`, since cli/root.go::signalExitCode depends on `errors.Is(err, context.Canceled)` for the SIGINT→130 mapping.

- **`smell:b5a79fda:deploy-phase-stringly-typed`** — done 2026-05-20 — PR #660, merge commit `22465f67`. Tier J minor (magic-strings). Replaced the three bare `prepare`/`install`/`configure` strings (producer in helpers.go, consumer in deploystate.go) with `type deployPhase string` + a const block, threaded through `markDeployPhase`/`markDeployPhaseFatal`/`writeDeployState` signatures and the `announceDeployState` switch. `encoding/json` marshals named string types by their underlying value, so existing on-disk marker files round-trip unchanged with zero codec overrides. Matches the prevailing repo enum pattern (`cleanup.Kind` / `bundleStatus` / `stepDisplayStatus`). **Postmortem lesson:** A named-string enum refactor over a JSON-serialized contract is wire-format-free precisely because `encoding/json` treats `type X string` identically to `string` for marshal/unmarshal — verifying this before adding a `MarshalJSON` override saved over-engineering.

- **`smell:fd2125dd:output-flag-magic-string`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J minor (magic-strings). Picked and claimed during /roadmap-pickup; the read-only planner found the item ALREADY resolved on develop. Commit `08d3e35` ("refactor(cli): centralise output flag and root-annotation constants") is an ancestor of the worktree base `b4307b7`: it renamed the old `flagOutput` (`"output-file"`) to `flagOutputFile`, added `flagOutput = "output"` + `flagOutputShort = "o"` with the typo-guard doc comment, and migrated all 8 format-flag registration sites. No bare `"output"`/`"o"` format-flag literals remain (the lone surviving `tui.LF("output", outPath)` in `debug_bundle.go` is a structured-log attr key, correctly untouched). The roadmap Fix proposed the constant names `flagOutputFormat`/`flagOutputFormatShort`; the landed names are `flagOutput`/`flagOutputShort` — the substance (named constant + typo-guard doc + all sites migrated) is satisfied, the spelling differs. Its sibling `ux:08ec0042` at roadmap line 779 already carries the explicit "already implemented in commit 08d3e35 — needs archive" annotation; this entry simply missed the same annotation. No worktree mutated, no PR opened. **Postmortem lesson:** the plan-then-execute split paid off again — a read-only planner using `git merge-base --is-ancestor` proved the fix predated the worktree base before any diff was attempted; when one item in a same-commit cluster carries an "already implemented" annotation, its untagged siblings must be verified against that commit too, not taken at face-value `not started`.

- **`dep:6ebdb617:claudemd-yaml-tripwire-stale`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (duplicate-engine). Picked and claimed during /roadmap-pickup; the read-only planner found the item ALREADY resolved on develop. Commit `3243cdd` (`docs(claude): correct yaml engine count to four`, cherry-picked from `cdbd529`, archived as `dep:33ef32bf:yaml-quad-engines` / PR #649) had already rewritten `CLAUDE.md` §Dependencies "YAML engine baseline (tripwire)" to the exact phrasing of this item's Fix option (a): "Four YAML engines are in the dep tree: sigs.k8s.io/yaml v1.6.0 (direct ...); go.yaml.in/yaml/v{2,3} (transitive ...); gopkg.in/yaml.v3 v3.0.1 (transitive, pulled via stretchr/testify → gopkg.in/check.v1) ... Do not add a fifth engine without a recorded justification here." The Evidence text quoted in this entry ("Count is down from four — gopkg.in/yaml.v3 was dropped from go.mod require") is the pre-PR-#649 state and no longer exists in CLAUDE.md. Option (b) (drop testify, return to three engines) was explicitly out of scope for a suggestion-level item. No worktree mutated, no PR opened. **Postmortem lesson:** Tier J's `dep:33ef32bf:*` cluster pre-emptively satisfied this entry, but the entry was filed pre-#649 and never amended; this is the third already-done item discovered in one /roadmap-pickup session (alongside `smell:fd2125dd` and `mod:eb479d86`). When a roadmap entry quotes verbatim source text in Evidence, a planner should grep for that exact text in the current tree before assuming the item is actionable — its absence is a fast tripwire that the fix has already landed.

- **`sec:35abd54e:env-string-residue`** — done 2026-05-20 — PR #662, merge commit `a1546bf`. Tier J major (credentials). `ProxmoxCredentials.Env()` converts `[]byte` Password/APIToken to immutable Go strings that the receiving `Executor.Env` retains after the underlying bytes are zeroed. PR #642 (`api:7b2829bb:zeroize-asymmetry`) had already added `executor.Executor.ZeroizeEnv()` and wired `defer prov.ZeroizeEnv()` at `install/phase.go:DeployInfrastructure`, but three other phase sites (destroy/helpers.go, postinstall/bootstrap.go, install/workers.go) built a local `terraform.Executor` via `terraform.WithEnv(p.Exec.Env)` and never zeroed the copy. Added `defer tf.ZeroizeEnv()` at each plus a CLAUDE.md §"Credentials and secrets" addendum naming short-lived `terraform.Executor` locals as in-scope for the defer pattern. The roadmap's "update proxmox.go" target was a non-issue (proxmox owns `p.env` zeroed via its own `Provider.ZeroizeEnv` deferred at `install/phase.go:139`); reviewer independently verified all six `terraform.New(` sites accounted for. Reviewed PASS round 1. **Postmortem lesson:** a roadmap Fix that lists multiple target files can be partially landed by prior PRs — the planner must grep the canonical method (here `executor.ZeroizeEnv`) before re-adding it; here the helper was already present and only the wiring at three remaining sites was needed.

- **`api:7b2829bb:exposed-fields-no-callers`** — done 2026-05-20 — PR #663, merge commit `d5d26af`. Tier J minor (exported-surface). Item 1 of a four-commit sequential chain on the executor/terraform shared surface (all four bundled into one PR because they serialize on `executor.go` / `proxmox.go`). `executor.Executor` exposed `Stdout`/`Stderr`/`WorkDir`/`Verbose` with no external mutators while the package committed to functional options. Unexported the four fields, completed the option API with `WithStdout(io.Writer)` / `WithStderr(io.Writer)`, and deleted `Verbose` entirely (grep proved zero readers; the only writes were two dead `t.exec.Verbose = t.Verbose` lines in terraform.go which were also removed). The roadmap's "external code imports these names today" claim was wrong: grep found 3 external `Stdout` sites in `setup/upload_test.go` (rewritten to `executor.WithStdout(&buf)`) plus the 2 dead terraform writes. **Postmortem lesson:** when a roadmap Fix says "no external code imports these names", treat it as a hypothesis to verify with grep; the cross-package test reads were caught only by grepping every receiver-method on the type, not by the roadmap's "no external" assertion.

- **`err:fde34e0c:exit-error-no-ctx-identity`** — done 2026-05-20 — PR #663, merge commit `e37906f`. Tier J minor (cancellation-identity). Item 2 of the executor/terraform chain. Every `&executor.ExitError{}` construction site built a typed error on non-zero exit without consulting `ctx.Err()`, so a SIGINT-cancelled subprocess produced no `context.Canceled` identity and `cli/root.go::signalExitCode` mapped the run to exit 4 (ClusterError) instead of 130. Added exported `executor.NewExitError(ctx, cmd, code, stderr) error` that returns `ctx.Err()` when cancelled; routed 5 direct construction sites through it (3 in executor.go, 1 in cluster/k8s.go, 1 in phase/kubectl.go) plus an early-ctx-guard at `setup/release_extract.go:128` where the typed `*executor.ExitError` value is reused in two error-wrap branches (canonical install/monitor.go shape — avoids type-assertion complexity from threading through the wrap). Updated the now-stale doc comments on `runCheck` and `OcOutput` to acknowledge the ctx-cancel return path. The roadmap's `setup/upload.go:L28` was a stale entry (that site uses `fmt.Errorf`, not `&executor.ExitError{}`). **Postmortem lesson:** when a fix swaps a typed-error constructor for one that may return a different sentinel under cancellation, every comment claiming "always returns *ExitError" must be tightened at the same time — keep the doc honest or the next reader gets the wrong invariant.

- **`api:d5915b0c:exec-env-direct-mutation`** — done 2026-05-20 — PR #663, merge commit `12a0220`. Tier J minor (package-boundary). Item 3 of the chain. `SetupKubeconfig` was the only production code mutating `p.Exec.Env` directly (`p.Exec.Env = append(p.Exec.Env, ...)`), bypassing the functional-options API and defeating any future invariant. Added `executor.Executor.AppendEnv(kvs ...string)` for controlled post-construction mutation and `executor.Executor.SnapshotEnv() []string` returning a defensive `make+copy` so callers cannot mutate internal state through a returned slice. Migrated `SetupKubeconfig` to `AppendEnv`, unexported `Executor.Env` to `env`, and rewrote four external `p.Exec.Env` readers (install/phase.go, install/workers.go, postinstall/bootstrap.go, destroy/helpers.go) plus terraform's internal `WithEnv` body (`e.exec.Env = append(...)` → `e.exec.AppendEnv(...)`) to use the methods. SnapshotEnv's doc comment records the credential-lifetime contract: copies handed to terraform/proxmox WithEnv constructors are not reached by a subsequent `ZeroizeEnv` on the phase executor, so callers must zero their own copies. The expected merge conflict with PR #662 (sec:35abd54e) on the three workers/bootstrap/destroy sites auto-resolved cleanly because #662's `defer tf.ZeroizeEnv()` and #663's `SnapshotEnv()` rewrite touched different lines. **Postmortem lesson:** when two sequenced PRs both touch the same files but at different line positions, GitHub's 3-way merge auto-resolves; only line-overlapping edits force manual reconciliation — sequencing the broader-blast-radius PR last (here #663 over #662) minimises the reconciliation surface.

- **`api:4c092fce:terraform-mixed-shape`** — done 2026-05-20 — PR #663, merge commit `7c8cdc1`. Tier J minor (option-consistency). Item 4 (final) of the chain. `terraform.Executor` exposed `WorkDir`/`VarFile`/`Verbose` alongside its functional-options API — half-opaque, half-public. Unexported the three fields; added `WorkDir() string` getter (sole external read site: `proxmox.go` apply plan-file). `Verbose` had become dead after A1 removed its two propagation writes in terraform.go (the only readers); deleted the field, the `WithVerbose` option, and its lone caller in `destroy/helpers.go`. Lint failed mid-flight (post-A4 first attempt) on a write-only `verbose` field flagged by golangci-lint's `unused`; the delta dropped the field+option+caller together rather than keep scaffolding for a non-existent reader. Same-package tests rewrote `Executor{WorkDir:...}` literals to lowercase; the external `destroy/helpers_test.go` rewrote three `&terraform.Executor{WorkDir: dir}` literals to `terraform.New(dir)` because cross-package struct-literal access to unexported fields would not compile. The new `WorkDir()` getter receiver was originally `(e *Executor)` but revive flagged receiver-naming inconsistency vs the rest of the type's `(t *Executor)` methods — corrected before commit. **Postmortem lesson:** unexporting a struct field with `golangci-lint`'s `unused` linter enabled means a write-only field will fail lint — either unexport AND delete the field+writer, or keep both exported; "half-rename" produces unused-field failures that only show up in the verification step.

- **`ux:08ec0042:flag-output-name-collision-risk`** — done 2026-05-20 — PR — (no PR), merge commit `08d3e35`. Tier J suggestion (flag-conventions). The roadmap entry's premise was that `flagOutput = "output-file"` collided with the `--output` format flag. Commit `08d3e35` ("refactor(cli): centralise output flag and root-annotation constants") had already split this: `flagOutput = "output"`, `flagOutputShort = "o"`, `flagOutputFile = "output-file"`, with the typo-guard doc-comment naming both. The eight format-flag registration sites use the new `flagOutput`/`flagOutputShort` constants. No additional work required. **Postmortem lesson:** sibling items in a roadmap cluster can be unevenly annotated — when one entry carries the "needs archive" tag, grep the constant names in the cited commit to confirm the others don't share the same status.

- **`mod:ddf885f4:use-slices-contains`** — done 2026-05-20 — PR — (no PR), merge commit `239959e`. Tier J minor (slices-maps). Commit `239959e` ("refactor: prefer slices.Contains over hand-rolled loops") landed the suggested refactor in `Manager.dependsOn` (`internal/addon/manager.go`) hoisting the equality short-circuit out via `slices.Contains` and iterating only to recurse. No additional work required. **Postmortem lesson:** an "already implemented" annotation in the roadmap is verifiable by grep + git-log lookup, but the entry stays pickable until somebody archives it; the read-only planner workflow makes the planner the choke-point for catching these.

- **`mod:c19ee328:slices-containsfunc-ignition`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (slices-maps). The audit proposed rewriting `StepGenerateIgnition.AlreadyDone` from a hand-rolled all-files-present loop to `!slices.ContainsFunc(...)`. A subsequent refactor introduced the `IgnitionSentinel` sentinel-file approach (`internal/distribution/okd/setup/ignition.go::IgnitionSentinel`, called from `setup/steps.go::AlreadyDone`) which replaces the entire all-files-present check with a single existence test on a completion marker. The audit's proposed rewrite is moot — the surface it targeted no longer exists. **Postmortem lesson:** audit findings can be superseded by orthogonal refactors that change the API surface; a follow-up audit pass would no-op the finding.

- **`dep:3295df72:transitive-test-deps-from-proxmox`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (transitive-weight). Audit-positive baseline: go.sum carries `h2non/gock`, `h2non/parth`, `go-test/deep`, `gopkg.in/check.v1` as transitive test-only deps from go-proxmox. None enter the release binary; none have okdctl call sites. The audit Fix is "No action — Go's MVS pulls these from go-proxmox's test imports." Cannot be pruned at the consumer side without dropping go-proxmox or applying a replace directive (which CLAUDE.md §dependencies advises against on unreached transitives). **Postmortem lesson:** transitive test-only deps are a permanent fixture of any project that consumes a dep with a test suite; documenting the acceptance is the durable artifact.

- **`dep:33ef32bf:dup-log-engines-stack`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (duplicate-engine — third echo). Re-confirmation of the prior `dep:33ef32bf:dup-log-engines` (Tier H, archived 2026-05-08) and `dep:33ef32bf:dual-log-engines` (Tier I, archived 2026-05-08) entries. Four log engines remain the steady state: stdlib `log/slog`, `charm.land/log/v2` (intentional TUI stack), `k8s.io/klog/v2` (transitive via apimachinery), `go-logr/logr` (transitive via klog). CLAUDE.md §Dependencies "YAML engine baseline (tripwire)" already documents the multi-engine policy; the log-engine equivalent is implicit. Audit Fix: "No action — record as steady state." **Postmortem lesson:** the same finding under a new audit sub-id (e.g. ":stack" suffix vs the bare ":dup-log-engines") archives as a re-confirmation rather than a new investigation.

- **`dep:33ef32bf:gorilla-websocket-stale`** (Tier J 2026-05-08) — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (maintenance-signal — third echo). Re-confirmation of the Tier H 2026-04-25 and Tier I 2026-05-05 entries of the same name (both archived 2026-05-08). `gorilla/websocket v1.4.2` is transitive-only via go-proxmox, zero okdctl call sites. CLAUDE.md §dependencies "Maintained but upstream-locked deps" already documents the wait-for-upstream policy. No action. **Postmortem lesson:** a `dep:*:stale` finding under the same sub-id has now been re-archived three times; the next audit should suppress re-flagging by filtering against CLAUDE.md's documented upstream-locked list.

- **`dep:33ef32bf:k8s-pseudoversion-floor`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (justified-version-floor). `k8s.io/kube-openapi`, `k8s.io/utils`, `sigs.k8s.io/json` use commit-hash pseudoversions because k8s.io/* sub-modules ship without tagged releases. This is the upstream norm; apimachinery does the same. The audit Fix calls for documenting a release-prep checklist instruction to bump all sub-modules in lockstep when `k8s.io/api` bumps; the maintainer policy is to handle this at the bump time rather than maintain a separate checklist doc. No code change. **Postmortem lesson:** version-floor pseudo-pin guidance lives in CLAUDE.md §dependencies (license policy + pin stability sections), not in a separate release-prep doc; release-prep checklists are owned by the release runbook, not by individual audit items.

- **`dep:33ef32bf:proxmox-bus-factor-reconfirm`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (maintenance-signal — third echo). Re-confirmation of `dep:33ef32bf:proxmox-v0x-bus-factor` (Tier H, archived 2026-05-08) and `dep:33ef32bf:proxmox-bus-factor-1` (Tier I, archived 2026-05-08). `luthermonson/go-proxmox v0.5.0` is the sole Proxmox API client; CLAUDE.md §dependencies already documents the bus-factor-1 caveat and the ~200-LOC REST fallback plan. No action. **Postmortem lesson:** same finding under a new audit sub-id ("-reconfirm" suffix) re-archives in seconds when the policy is already in CLAUDE.md; the read-only planner verifies the documented policy is still live before archiving.

- **`con:181efc90:spinner-canonical`** — done 2026-05-20 — PR #666, merge commit `8086b73`. Tier J suggestion (goroutine-lifetime). Added a two-line WHY anchor to `internal/tui/spinner.go::StartSpinner` naming audit id `con:181efc90` and the dual-stop-signal canonical pattern (`stopCh` via `sync.OnceFunc` + `ctx.Done()` + ordered `done` teardown). No code change. CI test-go passed clean on the first run. Reviewer PASS round 1. **Postmortem lesson:** an audit-positive marker on a 1-3 sentence doc-comment is a 2-line append, not a freestanding comment block — extending an existing doc block keeps the section index unchanged and the grep marker self-attached to the function.

- **`con:39c75e91:confirm-stdin-leak-bounded`** — done 2026-05-20 — PR #667, merge commit `53a1bce`. Tier J suggestion (goroutine-lifetime). Appended a single sentence ("Recorded as audit-positive baseline (con:39c75e91).") to `promptForConfirmation`'s existing Design-note comment in `internal/cli/confirm.go`. The function's bounded-leak shape (cap-1 channel, fire-and-forget stdin reader) was already documented; the new marker lets future audit passes grep the audit id and skip. Reviewer PASS round 1. **Postmortem lesson:** when the existing doc comment already covers the substance of an audit-positive claim, the minimal-information edit is appending the audit id to that block — not adding a parallel block that would echo the same content.

- **`con:48688e63:disconnect-ctx-ignored`** + **`api:48688e63:ctx-symmetry-no-network`** — done 2026-05-20 — PR #668, merge commit `ce92536`. Tier J suggestions (sibling pair, same file: `internal/infrastructure/proxmox/proxmox.go`). Anchored the ctx-symmetry scaffolding rationale on `Provider.Disconnect` (the authoritative comment) with a one-line Connect cross-reference. Names both audit IDs and roadmap constraint 1 (single provider: Proxmox); the `_ context.Context` receiver intentionally stays per MEMORY.md §scaffolding. Reviewer PASS round 1 acknowledged the 7-line comment but justified it on the three load-bearing invariants (intentional `_`, future-handshake intent, roadmap constraint 1 refutation). **Postmortem lesson:** two roadmap items targeting the same scaffolding pattern collapse into a single PR with one authoritative anchor location and a cross-reference — putting both rationales on both functions would double the comment weight without adding signal.

- **`con:8e65d574:bgcheck-canonical-leak-bound`** — done 2026-05-20 — PR #669, merge commit `7bedefb`. Tier J suggestion (goroutine-lifetime). Inserted a single re-confirmation line into `BackgroundCheck`'s existing doc comment in `internal/version/updatecheck.go`, recording the new audit id alongside the prior `con:8e65d574:update-check-bounded-leak-doc` baseline. The fire-and-forget pattern (cap-1 channel, httpTimeout=4s leak bound, 100ms caller wait) is preserved verbatim. Reviewer PASS round 1. **Postmortem lesson:** for a third+ audit re-confirmation on the same site, the marker form is "re-confirmed by <id>" rather than a fresh assertion — it keeps the comment from accreting one full line per audit cycle.

- **`con:aa84670c:signalloop-bounded-leak`** — done 2026-05-20 — PR #670, merge commit `9d12399`. Tier J suggestion (goroutine-lifetime). Added a two-line invariant comment to `signalLoop` in `internal/cli/root.go` recording the audit id and the load-bearing ordering: `close(sigCh)` after `signal.Stop` is what lets the receiver observe `!ok` on the happy path. Round 1 reviewer FAILed an earlier draft that used "canonical X / no fix required" framing (CLAUDE.md "self-referential peacock talk" antipattern + meta-commentary about the audit process). Round 2 rewording named the concrete invariant ("do not move or remove without re-deriving") and passed. **Postmortem lesson:** an audit-positive marker that asserts a quality claim ("canonical X") is peacock talk; the version that names a concrete invariant and the consequence of violating it ("ordering is load-bearing for the bounded-leak contract") is real WHY information. The reviewer caught the difference exactly.

- **`con:ae5b624c:monitor-cmd-cancel-pattern`** — done 2026-05-20 — PR #671, merge commit `0a78a28`. Tier J suggestion (goroutine-lifetime — seam→audit-subprocess). Appended a single line to `defaultStartMonitorCmd`'s doc comment in `internal/distribution/okd/install/monitor.go` naming audit id `con:ae5b624c` and the related `sub:7b2829bb` consumer site that mirrors this cmd.Cancel+WaitDelay shape. Reviewer PASS round 1 noted the cross-reference to sub:7b2829bb is what saves the line from "self-referential peacock talk" — naming a second site that depends on the shape carries real signal. **Postmortem lesson:** an audit-positive marker that names a CONSUMER of the pattern (here `sub:7b2829bb`) avoids the peacock antipattern by anchoring to a real refactoring contract — the canonical pattern claim becomes a load-bearing one because something else depends on it.

- **`api:c287d5c0:zeroize-no-callers-yet`** — done 2026-05-20 — PR #672, merge commit `0ef5568`. Tier J suggestion (exported-surface — seam→audit-security). Appended a single sentence to `Provisioner.ZeroizeEnv`'s doc comment in `internal/distribution/okd/okd.go` naming audit id `api:c287d5c0` and pointing at `executor.Executor.ZeroizeEnv` as the field owner. Per MEMORY.md §scaffolding the wrapper method is preserved despite its delegation-only body. The companion item `api:7b2829bb:zeroize-asymmetry` (PR #642, archived 2026-05-19) had already established `executor.Executor.ZeroizeEnv` as the canonical owner; this PR documents why the Provisioner wrapper survives. CI test-go failed with exit 143 (SIGTERM, runner-killed) on the first attempt — a doc-only change cannot break unrelated test packages, matched the MEMORY.md `ux:0d318f5c` flake signature, re-ran clean. Reviewer PASS round 1. **Postmortem lesson:** when an "already-implemented" companion has landed and the surviving wrapper is intentional scaffolding, the doc anchor names BOTH the audit id AND the field owner — without the field-owner pointer, a future reader still has to grep to find the canonical implementation.

- **`smell:08ec0042:flags-package-not-canonical`** — done 2026-05-20 — PR #665, merge commit `9ed6fac`. Tier J suggestion (helper-package-no-value). Extended `internal/cli/flags.go` with the seven previously-bare root-level flag-name constants (`flagConfig`, `flagConfigShort`, `flagLogLevel`, `flagLogFormat`, `flagLogFile`, `flagQuiet`, `flagVerbose`) and migrated the eight registration sites in `internal/cli/root.go`, the one read-back site in `internal/cli/logging.go`, and the four `flagOnly`/`flagTarget` sites in `internal/cli/destroy.go`. Per-command boolean tail flags (`--keep-isos`, `--skip-terraform`, `--confirm-cluster`, etc.) intentionally stay as bare strings — single-site registrations with no read-back risk per CLAUDE.md §architecture. Round 1 reviewer FAILed because the planner missed the four `flagOnly`/`flagTarget` sites in destroy.go where the constants already existed but bare strings were still used; round 2 delta closed those exactly. CI test-go failed once with `conclusion: null` on the `go test` step (runner died mid-execution, MEMORY.md `ux:0d318f5c` signature) — re-ran clean. **Postmortem lesson:** the typo-guard rationale only applies to flags referenced at BOTH registration and read-back; per-command bool tail flags with a single binding site don't need centralisation. A planner can usefully identify which bare strings are real targets vs intentional locals by looking at whether the flag has a `Changed()` / `Lookup()` / `MarkFlagsMutuallyExclusive()` second site.

- **`err:48688e63:cancel-identity-lost-on-tf-apply`** — done 2026-05-19 — PR #615, merge commit `9c1b114`. Tier J major (cancellation-identity — seam→audit-concurrency). On terraform-apply cancel, `internal/infrastructure/proxmox/proxmox.go:195-203` wrapped only the executor.ExitError, dropping context.Canceled identity downstream and breaking `cli/root.go::signalExitCode`'s SIGINT→130 mapping. Fix used `errors.Join(ctx.Err(), applyErr)` so both identities walk the chain — net 0 LOC, same return shape, exit-code taxonomy honoured. **Postmortem lesson:** when wrapping a subprocess error after `ctx.Done()` fires, prefer `errors.Join(ctx.Err(), subErr)` over `fmt.Errorf("...: %w", subErr)` — the join preserves both the ctx identity (for exit-code mapping) and the subprocess detail (for diagnostics).

- **`err:7b2829bb:exit-error-no-redact`** — done 2026-05-19 — PR #616, merge commit `a82db30`. Tier J major (redaction-in-error — seam→audit-observability). `executor.ExitError` (`internal/executor/executor.go:187-198`) carried a Stderr field but didn't implement `Redacted() any`, so subprocess stderr reached slog/tui sinks unredacted whenever oc/kubectl/terraform/sops failed. Fix mirrored `system.SubprocessError.Redacted` at `internal/system/exec.go:42-44`: added `Redacted() any` returning {Command, ExitCode} so logutil.RedactHandler's interface switch catches ExitError as an attr value; `terraform.ExecError` aliases ExitError and inherits automatically. Net +4 LOC. **Postmortem lesson:** when two sibling error types implement the same telemetry contract on the same wire (slog handler), asymmetric implementation creates silent data-class leaks at the asymmetric type — a one-method mirror is the minimal patch.

- **`ux:fd2125dd:args-validator-not-usageerror`** — done 2026-05-19 — PR #617, merge commit `b9d15d9`. Tier J major (exit-codes — seam→audit-errors). `addonInstallCmd.Args` in `internal/cli/addon.go:62-73` returned plain `fmt.Errorf` for missing/conflicting positional args, exiting 1 (generic) instead of 64 (EX_USAGE) per docs/cli/exit-codes.md. Fix wrapped each return in `&errtypes.UsageError{}` so `exitCodeFor()` resolves EX_USAGE, matching the SetFlagErrorFunc pattern in `root.go:256-259`. **Postmortem lesson:** positional-arg validation is the natural sibling of flag-parse validation in cobra's contract — both belong in the UsageError class, and a CLI that exits 1 for one shape but 64 for the other has a taxonomy hole the next cobra Args callsite will silently widen.

- **`sub:a6e38cc7:no-stderr-capture`** — done 2026-05-19 — PR #618, merge commit `12bb8f6`. Tier J minor (io-handling). `runKeyscan` in `internal/sshpin/sshpin.go:41-47` used `cmd.Output()`, so stderr inherited the parent fd and port-22-closed diagnostics landed on the operator's terminal but never reached the returned err. Fix swapped to `system.OutputCaptured(ctx, "ssh-keyscan", ...)` so stderr flows into a `*SubprocessError` matching sibling `RunCaptured` at `exec.go:73`. Same return shape, better error. **Postmortem lesson:** `cmd.Output()` is a footgun in this codebase — the canonical helper `system.OutputCaptured` already exists; any new `exec.Command` site that needs structured error context should reach for it first.

- **`sub:e2343d2c:no-cmd-env`** — done 2026-05-19 — PR #619, merge commit `1028f75`. Tier J minor (io-handling). Three exec.CommandContext sites in `internal/system/systemd.go:40-68` (`ManageService`, `IsServiceActive`, `IsServiceEnabled`) bypassed `executor.FilterParentEnv`, so under sudo re-exec they ran as root with unfiltered parent env. Fix routed all three through `system.RunCaptured`, letting it own the exit-code→bool conversion via `errors.As` against `*SubprocessError`. **Postmortem lesson:** when an env-isolation invariant exists in a package, every new exec site is a chance to violate it — the durable defence is to make the helper that owns the invariant (`RunCaptured`) the only sanctioned exec entry-point, not just the recommended one.

- **`smell:6424733c:metrics-shutdown-timeout-magic`** — done 2026-05-19 — PR #620, merge commit `4aeb490`. Tier J suggestion (magic-strings). `startMetricsServer` in `internal/cli/helpers.go:228-256` hardwired 5s/10s/60s timeouts as inline `time.Second` literals on `http.Server` plus a 5s shutdown drain. Fix lifted the five durations to package-level `metricsReadHeaderTimeout` etc. with a single doc block explaining the Prometheus-scraper rationale; behaviour identical. **Postmortem lesson:** per-call magic durations on `http.Server` construction are a regression class — naming them at package scope with one doc block lets a future tuner adjust one without re-reading the constructor or reverse-engineering the policy from comment hints.

- **`state:b5a79fda:deploy-state-marker-stale`** — done 2026-05-19 — PR #621, merge commit `ad84027`. Tier J minor (crash-recoverability). `announceDeployState` in `internal/cli/deploystate.go:24-86` read `.okdctl-deploy-state.json` on destroy entry with no TTL or cross-cluster check, so a marker left by a successful deploy whose `clearDeployMarker` failed could mislead operators on unrelated destroys weeks later. Fix added a `ClusterName` field (omitempty) to `deployState`, emitted marker age in the announce warning, ignored markers from a different cluster, and made the first `markDeployPhase` write fatal so a write-failed marker can't accumulate silently. **Postmortem lesson:** a state file used for partial-deploy detection needs both an identity field (ClusterName here) and a freshness signal (mtime/age); without both, the file's informative power decays asymmetrically — the noise it generates outlasts its signal.

- **`obs:eb479d86:sprintf-attr-value`** — done 2026-05-19 — PR #622, merge commit `72e832c`. Tier J minor (field-stability). `internal/distribution/okd/setup/upload.go:138` pre-rendered `totalSizeMB` via `fmt.Sprintf("%.1f", ...)` for a slog attr, emitting `"size_mb": "123.4"` (string) in JSON output instead of a number. The c07157e migration swept message-arg fmt.Sprintf but missed this attr-value form. Fix dropped the Sprintf wrapper and pushed precision-1 rounding down one level via `math.Round(x*10)/10`, passing the resulting float directly. Net 0 LOC. **Postmortem lesson:** a previous sweep that fixed a class of fmt.Sprintf misuse in slog message strings doesn't automatically cover the attr-value form — same antipattern, different syntactic position, same downstream tooling break.

- **`mod:8ea706f6:strings-lines-version`** — done 2026-05-19 — PR #623, merge commit `382f2f5`. Tier J suggestion (slices-maps). `getToolVersion` in `internal/distribution/okd/setup/tools.go:260-264` used `strings.Split(strings.TrimSpace(string(output)), "\n")` only to read the first line. Fix replaced with `for line := range strings.Lines(string(output)) { return strings.TrimSpace(line) }; return "unknown"` — no slice materialisation, ~3 LOC removed; the empty-iterator guard from `strings.Lines` makes the explicit `len(lines) > 0` check unnecessary. **Postmortem lesson:** `strings.Lines` shines exactly when the caller only needs the first (or last) line — the no-allocation iterator collapses the slice-materialise-then-index-zero idiom into a single ranged read.

- **`ux:daf5bee9:kubeconfig-status-msg-bypasses-tui`** — done 2026-05-19 — PR #624, merge commit `4d7d7f7`. Tier J minor (streams — seam→audit-observability). After `AtomicWrite` or merge success, `internal/cli/kubeconfig.go:72` printed status via `fmt.Fprintf(os.Stderr, ...)` instead of `tui.Info` — bypassing slog → RedactHandler, so any future attr added to that line would silently lose redaction. Fix replaced with `tui.Info("kubeconfig written", tui.LF("path", ...))` / `tui.Info("kubeconfig merged", ...)` so structured fields flow through RedactHandler. **Postmortem lesson:** a bare `fmt.Fprintf` on stderr is fine for shell scripts but wrong for any line that might gain a credential-bearing attr later; the redaction tax for using `tui.Info` is zero, and the durability for future attrs is permanent.

- **`doc:4c092fce:pkg-doc-name-echo`** (terraform) — done 2026-05-19 — PR #625, merge commit `bc93bbd`. Tier J minor (package-doc). `internal/infrastructure/terraform/terraform.go:1-2`'s package doc opened with a tautology ("It can be used by any infrastructure provider that uses Terraform."). Fix replaced the second sentence with the real contract: subprocess shape (PATH lookup + executor env-allowlist), state-file invariants (atomic writes via `system.AtomicWrite`), and the Init/Plan/Apply/Destroy/Output call surface. **Postmortem lesson:** package docs are a contract surface — replacing "what the package is" with "what callers can rely on" is the difference between godoc that documents and godoc that promises.

- **`doc:35abd54e:pkg-doc-thin`** (credentials) — done 2026-05-19 — PR #626, merge commit `958d14c`. Tier J suggestion (package-doc). `internal/credentials/proxmox.go:1` carried a one-sentence vacuous doc despite owning the load-bearing []byte/Zeroize/Redacted-interface lifecycle. Fix expanded to two-to-three sentences naming the Password/APIToken []byte typing (so callers can defer Zeroize), the Redacted() slog contract, and the env-then-config resolution priority via `LoadEnvFile` / `GetProxmoxCredentials`. **Postmortem lesson:** for packages whose invariants are stricter than their type signatures (credentials, secret-bearing slices), the package doc is the only place a caller learns the lifecycle contract before tripping over it — the type doc is too late.

- **`doc:d5915b0c:pkg-doc-name-echo`** (install) — done 2026-05-19 — PR #627, merge commit `1a6ab57`. Tier J suggestion (package-doc). `internal/distribution/okd/install/phase.go:1`'s doc echoed the package name with no signal added. Fix surfaced the phase timeline (terraform-up → bootstrap monitor → CSR approval → cluster-operator settle) and the default timeouts (30m bootstrap, 60m install, 30s CSR poll), all overridable via `Deployment.*` in `Config`. **Postmortem lesson:** a package whose name already explains its purpose still benefits from a doc — but the doc should add the timing/sequence/override surface, not echo what the name already says.

- **`sec:fde34e0c:k8s-kubeconfig-env-no-validate`** — done 2026-05-19 — PR #628, merge commit `fc6a2b0`. Tier J suggestion (input-validation). `cluster.WithEnvFallback()` in `internal/cluster/k8s.go:55-64` read `$KUBECONFIG` without validation — a user-controlled value pointing at `/dev/zero`, `/proc/self/environ`, or a 100MB malicious yaml would be consumed by every K8sClient. Fix added Lstat symlink-reject + `filepath.Clean` + a `$HOME` / `/etc` allowlist; rejection falls back to skipping the env value plus a Debug log; API shape unchanged. Bounded reachability (production callers pass `WithKubeconfig` explicitly) so flagged as hardening rather than a vulnerability. **Postmortem lesson:** an env-fallback constructor that's "for tools or future callers" still needs the same input validation as the project-rooted path — the symmetry keeps the security baseline from drifting between the two construction modes.

- **`smell:62cb8a95:state-major-bounds-misnamed`** — done 2026-05-19 — PR #629, merge commit `c8e6c71`. Tier J suggestion (magic-strings). `internal/distribution/okd/destroy/helpers.go:26-59` declared `stateMajorMin, stateMajorMax = 1, 1` — two constants whose values were identical and used in a range check that semantically meant equality. Fix collapsed to `const requiredTerraformMajor = 1` and `if major != requiredTerraformMajor`; when v2 lands, the rename to a real range type comes then. **Postmortem lesson:** symmetric min/max constants with identical values encode a contract the body doesn't enforce; the equality check is the truth, and the spelling should match it — keeping the misnamed-range form means a future tightener-vs-loosener change reads as the wrong intent.

- **`con:6424733c:metrics-stop-shutctx-background`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (ctx-todo). Audit-positive baseline: `startMetricsServer.stop` in `internal/cli/helpers.go:240-256` builds a 5-second shutdown ctx via `context.Background()` rather than the parent ctx; the surrounding comment at L241-L242 explains "by stop() time the parent ctx is already cancelled by SIGINT, and we need the 5s drain to complete." The justification comment satisfies CLAUDE.md §concurrency's "context.Background()/TODO() needs justification" rule. No code change. **Postmortem lesson:** a documented `context.Background()` divergence with a one-paragraph WHY in the code is the canonical shape — once the comment is in place, the next audit's "ctx.Background() reflagged" finding archives as audit-positive on the same merge cycle.

- **`mod:6b533f2d:slices-collect-projection`** — done 2026-05-20 — PR — (no PR), merge commit `develop`. Tier J suggestion (slices-maps). Audit explicitly proposes a `mapSlice[T,U]` helper only when a third+fourth callsite appears: "Don't land in isolation — there's no Go 1.x-specific reason to migrate." The two extant callsites (`ApprovePendingCSRs` in `internal/cluster/k8s_csrs.go:63-66`, `handleHostNetworkConversion` in `update_ingress.go:292-295`, `KindStrings` in `cleanup.go:43-50`) keep the `make + for-i` form; that's the audit's own recommendation. No code change. **Postmortem lesson:** an audit finding whose proposed fix carries a "don't land in isolation" qualifier archives at the no-action threshold — landing a generic helper just to satisfy a finding that flagged itself as optional grows the codebase for zero callsite-density benefit.

- **`smell:39c75e91:yes-no-magic-strings`** — done 2026-05-20 — PR #673, merge commit `c5725bb`. Tier J suggestion (magic-strings). `isConfirmResponse` in `internal/cli/confirm.go:60-62` previously encoded the truthy set as three string literals (`"y"`, `"Y"`, `"yes"`) and rejected `YES`/`Yes` for case-sensitivity reasons. Fix collapsed the body to `strings.EqualFold(response, "y") || strings.EqualFold(response, "yes")`; tests at `confirm_test.go:22,70-71` were inverted to assert the new case-insensitive behavior. Semantic shift: `YES`/`Yes` now confirm where they previously denied — operator-authorized. **Postmortem lesson:** the "needs maintainer decision" caveat on a roadmap item is the maintainer's signal that the finding's proposed fix carries a semantic shift; when the maintainer re-authorises, the right move is to land the proposed fix AND update tests in the same PR — splitting the test update into a follow-up leaves a brittle CI state.

- **`E4`**: SSH/SCP host-key pinning for Proxmox + **`E5`**: Flux SSH known-hosts fingerprint pinning — done 2026-05-20 — PR #674, merge commit `de6f10a`. Tier E (security, audit `sec:27088eab` / `sec:eb479d86` / `sec:98723e5d`). Bundled because both consumed `internal/sshpin` (already landed) and the symmetric `SHA256:<base64>` vocabulary. **E4**: added `ValidateSSHFingerprint` in `internal/config/validators.go` so a malformed `provider.proxmox.ssh_host_fingerprint` value fails at config-load time instead of producing a confusing `sshpin.Verify` mismatch error at runtime; the per-key fingerprint logic in `internal/sshpin/sshpin.go` was already in place from an earlier landed change. **E5**: `verifyKeyscanFingerprint` in `internal/addon/catalog/flux/flux.go:441-466` now takes `acceptHostKey bool` and fails closed when neither `addons.flux.settings.git_host_fingerprint` (existing setting, NOT the roadmap's `known_hosts_sha256` alias) nor `accept_host_key=true` is set; `filterKeyscanLines` strips ssh-keyscan banner/comment lines so the deploy-key Secret is byte-stable across runs; `ValidateSettings` rejects non-SHA256 fingerprints; fixture-based tests (`TestVerifyKeyscanFingerprint_{Match,Mismatch,EmptyExpected_FailClosed,EmptyExpected_AcceptHostKey}`, `TestFilterKeyscanLines`) close the non-determinism regression class the PR #117 postmortem flagged. The third attempt at E4/E5 succeeded after re-authorisation; the PR #142 maintainer-closure had been an operational call, not a technical regression — recoverable as the postmortem predicted. **Postmortem lesson:** soft-deferred postmortem entries (status carrying "first attempt closed by maintainer call") are recoverable when the user re-authorises, but the salvageable artifact is the design (per-key `ssh.FingerprintSHA256`), not the diff — re-implementing on top of what's already landed avoids re-litigating the closed PR's scope.

- **`E3`**: HTTPS ignition + pinned CA via `--ignition-ca` — done 2026-05-20 — PR #675, merge commit `310d854`. Tier E (security, audit `sec:00000001:http-ignition-pullsecret`). Self-signed ECDSA P-256 cert generated under `<projectRoot>/certs/ignition/` (`internal/distribution/okd/setup/cert.go`); apache HTTPS vhost on :443 dropped into `conf-available`/`conf.d` (`internal/distribution/okd/setup/apache.go::configureApacheHTTPS`); CA passed to `coreos-installer iso customize --ignition-ca <file>` so the live env trusts the HTTPS ignition server during first-boot; `BuildIgnitionURL{,ForNode}` switched to `https://` with port 443 default; `VerifyWebServer` validates served cert against on-disk PEM via `httputil.NewWithCA` (fails closed on bad PEM); cleanup removes both the cert dir and the apache `ignition-ssl.conf` drop-in. **Round 1 review FAIL**: the planner proposed `coreos.inst.ca=<base64>` as a karg — a non-existent kernel argument. The canonical embed mechanism is `--ignition-ca <file>` at iso-customize time. **Round 1 fix** swept the karg + base64 dance, added `mod_ssl` to the RHEL package list (separate rpm; only `a2enmod ssl` works on Debian), flipped `cfg.HTTPServer.Port` default from 8080 to 443 (so `BuildIgnitionURL` doesn't TLS-handshake against the dead :8080), tightened cert dir to 0o700, required `net.ParseIP(ip)` non-nil before generating SAN-less certs, deleted the now-dead `configureApachePort`/`configureSELinuxForApache` helpers, and hashed the cert PEM into `nodeISOFingerprint` so cert regen invalidates the per-node ISO cache. **Round 2 review FAIL**: a stale doc comment on `EnsureIgnitionCert` still asserted "nodes pin it via coreos.inst.ca" — same misconception in the user-facing godoc. Round 3 fixed the comment. Round 3 PASS. **Postmortem lesson:** when a security-critical mechanism gets corrected, ALL its anchored text (function doc, exported-API godoc, commit messages on later PRs) must move with it — a stale doc comment that names the wrong mechanism is a future-reader trap that no amount of correct code can compensate for, and the independent reviewer caught exactly this on round 2. The deeper lesson: planners can hallucinate kernel-argument names. The plan-then-execute architecture's value is exactly that the independent reviewer judges the diff against the roadmap scope without seeing the planner's reasoning — a reviewer briefed on "use this karg" would have rubber-stamped the same hallucination.

- **`obs:de572c63:key-inconsistent-casing`** — done 2026-05-21 — PR #696, merge commit `64897bf`. Tier K minor (field-stability). ConfigureSystemResolver logged its systemd-resolved branch under `dns:` while the NetworkManager branch used `resolver:`, splitting the same logical operation across two prefixes. Renamed the two `dns:` occurrences in dnsmasq.go at L207/L238 to `resolver:`; dns.go's other call sites keep `dns:` for the dnsmasq-config concern. **Postmortem lesson:** when two code branches implement the same logical operation, the log prefix should reflect the operation, not the implementation — a downstream grep contract is a real interface, not a stylistic choice.

- **`obs:25fa1be8:level-warn-should-info`** — done 2026-05-21 — PR #697, merge commit `fa027a7`. Tier K suggestion (level-discipline). firewall.go:138 raised Warn for an expected no-backend state on hosts without firewalld/ufw/iptables; the function then no-ops and returns nil. Demoted to Info and added the `firewall:` prefix (the only line in the file lacking it). **Postmortem lesson:** Warn is for recoverable degradation, not for absence of an optional dependency; the `firewall:` prefix policy is enforced by review, not lint — drift surfaces on level-discipline audits.

- **`obs:8154ab0f:level-error-not-user-visible`** — done 2026-05-21 — PR #698, merge commit `b3ccf79`. Tier K suggestion (level-discipline). runDoctor emitted `tui.Error("doctor: failing checks block deploy")` immediately before returning `*errtypes.ConfigError`; cli/root.go::execute() then emitted `tui.Error("command failed", "err", err)` — two Error lines per failure event. Demoted to Warn so the cli layer's Error is the sole user-visible signal. **Postmortem lesson:** when a function returns a typed error that the cli layer will render as Error, the function itself should log at Warn — double-Error noise looks like two distinct failures to the operator.

- **`iac:85dfab4d:sh-curl-no-tls-floor`** — done 2026-05-21 — PR #699, merge commit `e7ae19c`. Tier K minor (install-sh-integrity). scripts/update-coreos-pins.sh's single curl invocation lacked the `--proto =https --tlsv1.2 --connect-timeout 10 --max-time 60` hardening that install.sh wraps via curl_safe. Inlined those flags on the raw.githubusercontent.com fetch; git ls-remote at L31 stays untouched (git's own TLS chain). **Postmortem lesson:** a CI helper script that pins SHAs is on the supply-chain critical path even when it never runs on user machines — apply install.sh's curl_safe discipline to every script that fetches and trusts external content.

- **`dep:76ed074a:make-air-pin-rationale`** — done 2026-05-21 — PR #700, merge commit `9c9df8b`. Tier K suggestion (maintenance-signal). Makefile:60 installed `air@v1.61.7` without a rationale comment, so a future contributor could not tell at a glance whether the pin was intentional or stale. Added a one-line comment noting that air is dev-only (not in the release binary) and that the pin is deliberate per the "never @latest" rule. No version bump. **Postmortem lesson:** a pinned version with no rationale invites a "bump to latest" PR; a one-line comment naming the risk boundary (dev-only) is cheaper than the post-bump review.

- **`con:660d83a5:progressbars-active-non-atomic`** — done 2026-05-21 — PR #702, merge commit `d170f3b`. Tier K minor (waitgroup-vs-errgroup). `progressBarsActive bool` was a plain bool read by goroutines spawned after ConfigureLoggers wrote it, while every peer field in the same var block was atomic.Pointer. Replaced with `atomic.Bool`; init() block now does `progressBarsActive.Store(true)` to preserve the default-true behaviour the var declaration formerly carried. **Postmortem lesson:** the race detector is silent when no two writes overlap, but the type still signals the wrong invariant to future readers — type to the worst expected case, not the current call graph.

- **`err:262af6e4:dnsmasq-path-error-not-typed`** — done 2026-05-21 — PR #703, merge commit `e02b3c5`. Tier K suggestion (sentinel-vs-typed). The dnsmasq AlreadyDone closure in cleanup.go:221-230 returned the bare error from dns.DnsmasqConfigPath; every sibling closure in the same file wraps validation failures in `*errtypes.ConfigError`. Wrapped the error as `&errtypes.ConfigError{Msg: "resolve dnsmasq config path", Err: err}` so exit-code mapping is uniform across the orchestrator and any direct-test caller. **Postmortem lesson:** orchestrator-only error wrapping is a silent dependency on a single entry point — type errors at their origin so the contract holds across all callers.

- **`err:1e8ffb91:postinstall-verify-bare-fmt-errorf`** — done 2026-05-21 — PR #704, merge commit `68e947c`. Tier K minor (sentinel-vs-typed). verifyKubeVIPAPIHealthBootstrap (verify.go:223-273) had five return paths; three already used `&errtypes.ClusterError` but two returned bare `fmt.Errorf`. Wrapped both bare exits to match siblings. The outer VerifyKubeVIP wrapper already typed the result, so observable exit codes were correct only via that wrap — typing at origin removes the silent dependency. **Postmortem lesson:** when 3-of-5 exit paths in a function use a typed error, the remaining 2 are not a stylistic choice — they're a bug-shaped gap waiting for the orchestrator wrapper to change.

- **`state:62cb8a95:destroy-snapshot-after-init-not-pre`** — done 2026-05-21 — PR #705, merge commit `4db82fa`. Tier K minor (tf-state-atomicity). destroyInfrastructure called `tf.Init()` then `tf.SnapshotState()`; init may rewrite `terraform_version` on schema migration, making the snapshot a post-init artefact rather than a pre-run restore point. Moved SnapshotState to run immediately after the HasState check, before Init. **Postmortem lesson:** "snapshot the state file" must mean "before any tooling touches it" — terraform init is not a no-op when a tfstate is present, so order matters.

- **`sec:88fd3050:input-url-scheme-not-checked`** — done 2026-05-21 — PR #706, merge commit `83639ac`. Tier K minor (tls-network). ProxmoxConfig.Insecure silently disabled TLS verification across every Proxmox API call via PROXMOX_VE_INSECURE=true with no log or warning — a typo or copy-pasted config flipped TLS off invisibly. Added a `tui.Warn("proxmox: TLS verification disabled (insecure=true in config)")` in reportCredentialProvenance so the warning surfaces once per okdctl invocation before any terraform run. **Postmortem lesson:** boolean security toggles deserve a visible provenance trail — the existing env-overrides-config warnings established the pattern, only Insecure was missing.

- **`sec:e3782ee7:env-file-perms-loose`** — done 2026-05-21 — PR #707, merge commit `edc70ca`. Tier K suggestion (credentials). WriteAsInvokingUser accepted any mode and forwarded to AtomicWrite without a credential-bearing ceiling; the sole production caller already passes 0o600, so the gap was forward-looking. Added a two-line doc note: credential-bearing files (kubeconfigs, pull secrets, private keys) must pass 0o600 or use CopyFileMode + ChownToInvokingUser. No signature change. **Postmortem lesson:** when the live caller set is safe but the helper boundary doesn't enforce the contract, a doc note is the lowest-cost guard — cheaper than a Sensitive bool, and bridges the gap until a linter rule lands.

- **`sec:98723e5d:toctou-chmod`** — done 2026-05-21 — PR #708, merge commit `5dc0cbe`. Tier K minor (file-toctou). addKubeconfigToBashrc did `os.Lstat` to refuse symlinks, then `os.ReadFile` — a window where an attacker could swap the regular file for a symlink and redirect root's read. Replaced ReadFile with `os.OpenFile(_, O_RDONLY|syscall.O_NOFOLLOW, 0) + io.ReadAll`; kept the Lstat as defense-in-depth for a cleaner diagnostic before ELOOP. Mirrors runlock.Acquire's pattern. **Postmortem lesson:** every Lstat-then-Open sequence is a TOCTOU unless the Open carries O_NOFOLLOW — the Lstat is for diagnostics, the O_NOFOLLOW is the actual guard.

- **`sub:451be4fa:no-cmd-env`** — done 2026-05-21 — PR #709, merge commit `3fe36cc`. Tier K suggestion (io-handling). HasPasswordlessSudo (advisory probe called only by doctor) skipped `cmd.Env`, so the subprocess inherited the full parent environment — inconsistent with every other subprocess site in internal/system. Added `cmd.Env = executor.FilterParentEnv(executor.DefaultEnvAllowlist)` for hygiene. Sudo's secure_path already clamps leakage in practice; this makes the implicit "every subprocess site uses the allowlist" rule actually total. **Postmortem lesson:** an inconsistency between probe subprocess sites and operational subprocess sites is a footgun waiting for a copy-paste — make the rule total even when the current call graph is safe.

- **`err:881d089e:runlock-bare-msg-no-err`** — done 2026-05-21 — PR #710, merge commit `5034022`. Tier K suggestion (wrapping). Acquire returned `&errtypes.ConfigError{Msg: msg}` on EBUSY without setting `Err`, dropping the syscall.Errno from the chain — callers could not `errors.Is(err, syscall.EWOULDBLOCK)` to distinguish lock-held from other failures. Added `Err: err` to that ConfigError. No exit-code change; ConfigError.Unwrap now walks to syscall.Errno. **Postmortem lesson:** when a typed error wraps a syscall failure, drop nothing — the Errno carries programmatic identity that string-match in Msg cannot replicate.

- **`mod:696d6b0e:use-strings-splitseq`** — done 2026-05-21 — PR #711, merge commit `5a4e502`. Tier K minor (slices-maps). Three iterate-once sites materialised `strings.Split(...)` into slices then ranged over them: iso_cleanup.go:212, install/workers.go:92, secretstore/providers.go:123. Replaced with `strings.SplitSeq` — no slice allocation, same loop body. A fourth site at iso_cleanup.go:289 was deliberately left as Split because the caller needs a materialised []string. **Postmortem lesson:** iterator-form replacements are mechanical only when the original loop iterates once and discards the slice — check the call site's downstream use of the result before swapping.

- **`state:c19ee328:setup-deploy-ignition-precondition-weak`** — done 2026-05-21 — PR #712, merge commit `3d4a94a`. Tier K minor (phase-idempotency). StepDeployIgnition.AlreadyDone checked only ignitionFilenames[0] (bootstrap.ign); a partial-write that died after the first .ign would set AlreadyDone=true on the next run, leaving master.ign/worker.ign absent and the cluster boot path broken. Strengthened the check to iterate every entry and return false on the first missing file. **Postmortem lesson:** when a step writes N files atomically (per-file but not as a set), the precondition must verify N, not 1 — the "first file as sentinel" trick only works if all writes are bundled under a single rename/sentinel.

- **`sec:8e65d574:dl-no-signature`** — done 2026-05-21 — PR #713, merge commit `b596033`. Tier K suggestion (tls-network). BackgroundCheck surfaced a "new version available" notice based on api.github.com's tag_name with no signature verification — operators could treat it as a trustworthy install signal. Added an explicit early-return for `Version == "" || Version == "dev"` so non-release builds don't surface spurious notices, and appended the canonical install.sh URL (`raw.githubusercontent.com/qxtaiba/okdctl/main/scripts/install.sh`) under the version line so operators follow the cosign-verified install path. **Postmortem lesson:** an in-app upgrade notice is a trust signal that must point at the verified path, not just announce a tag — silence-when-not-release prevents dev binaries from nagging.

- **`sec:06f00bcb:input-path-not-prefix-checked`** — done 2026-05-21 — PR #714, merge commit `bff4fe2`. Tier K minor (input-validation). validateHTTPServer only checked Port; HTTPServer.IgnitionServerIP and Root flowed verbatim into Apache vhost config via fmt.Sprintf, so a malicious YAML value could inject Apache directives. Extended the validator to require IP via netip.ParseAddr (through existing IsValidIP) and Root to be absolute with no shell/Apache metacharacters (newline, quotes, backtick, dollar, semicolon, angle brackets, backslash). Added FieldHTTPServerRoot and test cases for both fields. **Postmortem lesson:** every YAML-sourced string that lands in a templated config text needs a validator at the config boundary, not just a downstream consumer check — the config layer owns the typing decision.

- **`err:cfcdee2d:httputil-tls-sentinels`** — done 2026-05-21 — PR #715, merge commit `87a837d`. Tier K suggestion (sentinel-vs-typed). capRedirects returned `errors.New(...)` string-shaped errors for two distinct security failure modes (redirect cap, cross-host Authorization) — callers could only distinguish them via string match. Defined exported package-level sentinels `ErrTooManyRedirects` and `ErrCrossHostAuthHeader` and returned them so `errors.Is` resolves the security-relevant branch. **Postmortem lesson:** an `errors.New` returned from a security-relevant boundary is a string-match landmine — sentinels at the boundary cost one line each and unlock programmatic introspection forever.

- **`sec:c8b28673:zip-slip`** — done 2026-05-21 — PR #716, merge commit `cc29260`. Tier K minor (file-toctou). processTarEntry's TypeSymlink branch passed the textual prefix check but did not re-verify after `os.Symlink` succeeded — a previously-extracted parent symlink could redirect the new link's resolved path outside cleanDest. Added a post-Symlink `verifyResolvedPath(targetPath, cleanDest)` call that EvalSymlinks the just-created link, catching escapes through planted parent symlinks. Defense-in-depth on top of the existing O_NOFOLLOW guard. **Postmortem lesson:** textual prefix checks on extraction paths are necessary but not sufficient — every symlink creation in a tar extractor needs a post-creation real-filesystem resolution check, because EvalSymlinks sees what `filepath.Clean` cannot.

- **`tst:7b2829bb:snapshot-env-untested`** — done 2026-05-21 — PR #717, merge commit `b39698c`. Tier K **major** (cred-path-untested). executor.SnapshotEnv's copy-vs-reference invariant was doc-asserted but not test-locked — a regression to `return e.env` (alias instead of copy) would race a downstream ZeroizeEnv against the producer. Added TestSnapshotEnv with three sub-tests: copy independence (mutate snapshot, assert internal env unchanged), length match, and empty-env returns non-nil zero-length slice. **Postmortem lesson:** credential-bearing path invariants documented in doc comments must be test-locked — a doc says what the author intended, a test prevents future contributors from accidentally breaking it.

- **`iac:18a795d5:hcl-master-disk-no-ignore-changes`** — done 2026-05-21 — PR #676, merge commit `f122804`. Tier K major (hcl-destroy-ordering — seam→audit-state-and-recovery). The master VM `lifecycle.ignore_changes` block at `infrastructure/terraform/modules/proxmox-okd/main.tf:258-268` listed network_device/startup/cdrom/boot_order/efi_disk but omitted `disk`, while the sibling worker block at line 393 listed `disk` for the documented reason in `variables.tf:88-92` — lowering `master_data_disk_size_gb` would silently destroy the Ceph data disk on apply. Added `disk,` to the master ignore_changes list with a matching comment. One-line change, no behavior shift for unchanged values. **Postmortem lesson:** when two resource blocks share the same destructive-replacement risk (the variable doc itself warned about it), asymmetric `ignore_changes` lists are a bug — symmetry should be enforced by linter or audit, not by hope.

- **`doc:b3356305:readme-default-drift-fingerprint`** — done 2026-05-21 — PR #677, merge commit `d66f28a`. Tier K major (readme-drift). README §"SSH/SCP host-key trust on first run" framed `proxmox.host_fingerprint` as a future enhancement and listed only stopgap mitigations, but the field already shipped as `provider.proxmox.ssh_host_fingerprint` plus `provider.proxmox.require_pinned_fingerprint` per `internal/config/cluster.go:132-143` — operators following the README believed pinning was unavailable when it was a config edit away. Replaced the "Mitigations until …" paragraph with documentation of the existing fields, SHA256 format guidance, and `require_pinned_fingerprint` for fail-closed behaviour. **Postmortem lesson:** README "future enhancement" framing is a tripwire — when the feature lands, its absence from the README costs every operator who relied on the README's roadmap to predict capabilities.

- **`err:a6e38cc7:sshpin-keymatch-bare-errorf`** — done 2026-05-21 — PR #678, merge commit `d1f2f23`. Tier K major (sentinel-vs-typed — seam→audit-cli-ux). On host-key mismatch sshpin returned a bare `fmt.Errorf` at `sshpin.go:85-87`; the sibling "pinned but missing" branch at L78 already used `&errtypes.AuthError`. classifyStepErr therefore mapped key-pin mismatch to ClusterError (exit 4) instead of AuthError (exit 5), exactly the auth-failure surface exit 5 should flag. Replaced with `&errtypes.AuthError{Msg: fmt.Sprintf(...)}` — fingerprints are public values so Msg-interpolation is safe per the errtypes credential-substring scan. **Postmortem lesson:** when two branches in one function return errors with different security taxonomy implications (auth vs cluster failure), they must share the same typed-error class — otherwise the operator sees an exit code that lies about what happened.

- **`state:b804b2ec:bootstrap-sentinel-after-apply`** — done 2026-05-21 — PR #679, merge commit `058ba6d`. Tier K major (crash-recoverability). CleanupBootstrap at `postinstall/bootstrap.go:73-93` ran `tf.Apply` (destroying the bootstrap VM) before writing `bootstrap-state.auto.tfvars.json` with `bootstrap_enabled=false`. A crash or SIGKILL between apply success and the sentinel write left the VM destroyed but tfvars still claiming the VM should exist — the next plan would attempt to RECREATE it, requiring manual operator cleanup. Inverted the order: write the sentinel before Apply runs. If Apply fails the sentinel is harmless because terraform state still tracks the VM as present, so the next plan is a correct retry. **Postmortem lesson:** declarative-intent sentinels must precede the action they declare — otherwise the gap between "I intend X" and "X happened" is a crash window that turns one destructive op into a misleading retry.

- **`state:15ba17da:destroy-iso-removed-tf-failed`** — done 2026-05-21 — PR #680, merge commit `e9127c2`. Tier K major (destroy-safety). When StepDestroyInfra failed (`destroy/steps.go`), file cleanup was already downgraded to WorkOnly preserving tfvars — but StepRemoveRemoteISO ran before StepCleanupFiles in the sequence and had already removed ISOs that the (still-alive) Proxmox VMs may have needed for their next boot. ANDed the StepRemoveRemoteISO SkipWhen predicate with `!t.terraformFailed()` mirroring the existing StepCleanupFirewall pattern at line 158, and inlined the SkipReason as a static string listing all three causes (matches the firewall step's pattern). Removed the now-unused isoSkipReason helper. **Postmortem lesson:** step-ordering safety is not just about happy-path order — the SkipWhen of every destructive step must consider whether earlier steps in the same run already failed and left the world in a recoverable shape.

- **`err:ddf885f4:cluster-csr-stderr-in-msg`** — done 2026-05-21 — PR #681, merge commit `ff1dec8`. Tier K major (redaction-in-error — seam→audit-observability). PendingCSRs at `internal/cluster/k8s_csrs.go:20-24` interpolated raw `result.Stderr` from `oc get csr` into ClusterError.Msg via string concatenation, then returned that for any error sink to stringify. If kubeconfig carried an embedded bearer token (a documented kubectl shape) and oc surfaced it in stderr on auth errors, the token reached every printf path. RedactHandler scrubs structured attrs, not pre-rendered message bytes, so this was a real leak vector. Wrapped stderr in `executor.NewExitError(ctx, c.CLI+" get csr", result.ExitCode, result.Stderr)` and set it as ClusterError.Err — ExitError.Redacted() omits Stderr when slog encounters the error as a structured attr. **Postmortem lesson:** the "no Go error to wrap" rationale that justified the original concat was the smell — ExitError exists for exactly this case; any time you find yourself interpolating subprocess stderr into an error Msg, the wrapper type is the right shape, not the Msg.

- **`sub:97cb8adf:no-cancel-func`** — done 2026-05-21 — PR #682, merge commit `46e45ac`. Tier K major (timeout-cancel — related: sub:ae5b624c). RunCaptured and OutputCaptured at `internal/system/exec.go:52-86` built their `*exec.Cmd` without `cmd.Cancel` or `cmd.WaitDelay`. On ctx cancellation the os/exec default sent SIGKILL immediately, hard-killing apt-get/dnf/gpg mid-transaction with no flush window. The canonical executor.Executor already set Cancel=SIGINT + WaitDelay=30s for terraform's documented soft-cancel; the system/exec.go shim was never updated. Added `cmd.Cancel = SIGTERM` (correct for non-terraform binaries) and `WaitDelay = 30s` to both helpers with a WHY comment explaining why SIGTERM differs from terraform's SIGINT. **Postmortem lesson:** when two subprocess-runner shapes coexist (a feature-complete executor.Executor and a thin wrapper for simple cases), drift between them is a footgun — the wrapper inherits the obvious-good-default trap because nobody re-derives it.

- **`ux:08c49fc4:confirm-cluster-missing`** — done 2026-05-21 — PR #683, merge commit `f991865`. Tier K major (flag-conventions). updateIngressCmd is operationally destructive (deletes and recreates IngressControllers, ~30s outage per its Long doc) and exposed `--yes` to skip prompts, but lacked the `--confirm-cluster` typo guard that destroy/cleanup/addon uninstall all require with `--yes`. A scripted invocation in the wrong directory would outage the wrong cluster's ingress with no safeguard. Added the flag and a `confirmClusterMatches(updateIngressYes, updateIngressConfirmCluster, cfg.Cluster.Name, "update-ingress")` call in `runUpdateIngress` before any mutation. Regenerated `docs/cli/okdctl_update-ingress.md` for the new flag. **Postmortem lesson:** the typo-guard discipline is per-command — a destructive verb without it is the security parity gap; CI's docs-go check is the wire that catches missing doc regeneration when a flag lands.

- **`sec:ab9b764a:input-path-not-prefix-checked`** — done 2026-05-21 — PR #684, merge commit `7d2a768`. Tier K major (file-toctou). GenerateInstallConfig at `internal/distribution/okd/setup/ignition.go:47-57` read cfg.Files.PullSecret and cfg.Files.SSHPublicKey via bare `os.ReadFile`. Under the sudo re-exec model okdctl runs as root, so a user-planted symlink at either path redirected root's read to attacker-chosen content (e.g. /etc/shadow), baking those bytes into install-config.yaml served via the Apache ignition vhost. Added a package-private `readNoFollow` helper mirroring runlock.Acquire's lstat-then-O_NOFOLLOW pattern; routed both reads through it; preserved AuthError for pull-secret and ConfigError for SSH key. Added TestGenerateInstallConfig_SymlinkRejected covering both inputs. **Postmortem lesson:** every file path that crosses the user→root trust boundary needs the lstat+O_NOFOLLOW double guard; this is the third sec:* finding with the same pattern in Tier K, suggesting a lint rule (forbid `os.ReadFile` in setup/install paths) would close the class.

- **`sec:40d315ad:input-path-not-prefix-checked`** — done 2026-05-21 — PR #685, merge commit `77606e1`. Tier K major (file-toctou). createDeployKeySecret at `internal/addon/catalog/flux/flux.go:399-416` read `~/.ssh/flux-deploy-key` (private and .pub) via plain `os.ReadFile` after a `system.FileExists` existence check. Under sudo re-exec a malicious user-owned symlink at the deploy-key path redirected root's open to /root/.ssh/id_ed25519 or /etc/shadow, embedding those bytes into a flux-system Kubernetes Secret shipped to the cluster. Replaced the existence check with `os.Lstat` (refuses symlinks), introduced a `readKeyFile` helper applying O_NOFOLLOW at the open, and routed both L403 and L412 reads through it. **Postmortem lesson:** the addon catalog inherits the same sudo-re-exec trust boundary as the setup phase — every catalog/* file-read needs the same hardening, and the deploy-key pattern is widespread enough that a shared `internal/system` helper would prevent the next occurrence.

- **`state:761e5126:remove-haproxy-config-before-verify`** — done 2026-05-21 — PR #686, merge commit `2b3b69e`. Tier K major (destroy-safety — related sibling: state:632c9087). RemoveHAProxy at `postinstall/haproxy.go:47-113` stopped haproxy, removed /etc/haproxy/haproxy.cfg, cleared firewall, released VIP, then verified api-via-vip and api-via-hostname. If either verify failed the config was already gone and haproxy was stopped — the operator had to rebuild from setup. Moved both verifies into a pre-flight block at the top: with haproxy still running the VIP check hits kube-vip directly and the hostname check proves dnsmasq routes api.* to the VIP; if either fails RemoveHAProxy aborts before any destructive op. Copy haproxy.cfg to `.backup.<timestamp>` (matching the glob `cleanup/services.go:64` already sweeps) before the removal so the sibling rollback path has a recovery target. Added TestRemoveHAProxy_BackupCreated. **Postmortem lesson:** verifications belong before destructive ops, not after — "verify success" written after the destructive line is a post-mortem, not a guard.

- **`state:632c9087:update-ingress-haproxy-rollback-missing`** — done 2026-05-21 — PR #687, merge commit `f0ba062`. Tier K major (destroy-safety — sibling of state:761e5126). finalizeIngress rolled DNS back to bootstrap when RemoveHAProxy failed, but RemoveHAProxy had already deleted /etc/haproxy/haproxy.cfg by the time the error surfaced — clients pointed at bastion via the restored DNS then hit nothing. With sibling state:761e5126 landing the `.backup.<timestamp>` file before destruction, the rollback path now globs `haproxyConfigPath + ".backup.*"`, picks the most recent, and restores it via system.AtomicWrite (0o600). A single "rollback complete" Info log fires only when both DNS rollback and config restore succeed. **Postmortem lesson:** sibling-pair PRs require coordinated naming conventions — a glob in one PR must match the timestamp format the sibling writes, or the rollback path looks at the wrong files; the orchestrator (not the planner) is the only place where two planners' assumptions can be reconciled.

- **`ux:024a2c32:json-schema-undoc`** — done 2026-05-21 — PR #688, merge commit `66a44cb`. Tier K major (json-stability — seam→audit-documentation). `docs/cli/json-schema.md` documented Version, APIServerURL, ConsoleURL, Conditions, and Message as part of the stable `okdctl status --output=json` contract, but runStatus in `internal/cli/status.go:209-215` never set any of them (the ClusterStatus literal sets only Phase, APIReachable, Nodes, DegradedOperators, Addons). The omitempty tags silently dropped the absent fields, so the doc advertised five fields the runtime never emitted. Trimmed the five fields from ClusterStatus and from the doc's JSON example and field table — the implement alternative (3 oc subprocess calls + clusterversion JSON parsing) was over-budget. **Postmortem lesson:** an omitempty-tagged field on a public JSON schema is never an "optional" field — it's a contract claim that the runtime can produce that field; if the runtime cannot, omitempty is the silence that lies, and either the runtime or the doc has to change.

- **`tst:a3577f6c:ignition-cert-keygen-untested`** — done 2026-05-21 — PR #689, merge commit `839faa9`. Tier K **blocker** (cred-path-untested). EnsureIgnitionCert at `setup/cert.go:44-148` generated an ECDSA P-256 self-signed CA + private key, wrote the key at 0o600 inside a 0o700 dir, and that cert doubled as the trust anchor embedded into every node ISO. No test locked the perm bits, the reuse-when-valid/regen-when-IP-changes/regen-when-expired branches, or the x509 round-trip. Added cert_test.go with six tests covering perm bits (root-skipped), expired-cert returns ok=false, IP-SAN mismatch returns ok=false, matching IP-SAN returns ok=true, non-IP arg rejected, x509 round-trip verifies BasicConstraintsValid + IsCA. **Postmortem lesson:** when a credential-bearing helper's invariants are documented but not test-locked, a future refactor that swaps a write mode or a Lstat check passes silently — blocker-severity test gaps deserve six tests, not one happy-path test, because the regression surface is the multiplicative perm × expiry × identity × atomicity.

- **`tst:4c092fce:tf-snapshot-state-untested`** — done 2026-05-21 — PR #690, merge commit `c8bff52`. Tier K **blocker** (destructive-untested — seam→audit-state-and-recovery). SnapshotState (`internal/infrastructure/terraform/terraform.go:395-414`) is the only safety net between `okdctl destroy` and a destructive `terraform destroy` — it copies terraform.tfstate to a timestamped .bak before destroy fires; pruneSnapshots bounds retain to 5; checkStateMajorVersion gates schema upgrades. Nothing tested these. A regression dropping the AtomicWrite hard-error would silently proceed with destroy and lose the only recovery path. Added state_test.go with six tests: backup byte-equality + 0o600 perm, absent-source returns ("", nil), AtomicWrite-failure propagates wrapped error, pruneSnapshots keeps the 5 newest, checkStateMajorVersion rejects major=2 as `*errtypes.ConfigError`, unparseable version is non-fatal. **Postmortem lesson:** when a single function is the only safety net for a destructive op, the test count should equal the number of guarantees the doc comment claims — anything less is a documentation-vs-implementation drift waiting to happen.

- **`tst:a55b4592:loader-perm-gate-untested`** — done 2026-05-21 — PR #691, merge commit `2a0788a`. Tier K major (cred-path-untested). Loader.LoadFile at `internal/config/loader.go:27-55` enforces a `&0o022` group/world-writable perm gate and rejects unknown schemaVersion / unknown top-level keys before parsing — credential-handling-adjacent guards (config carries Proxmox host + may carry pull-secret paths). None of the three guards were tested. Added loader_test.go with table-driven perm-gate cases (0o600/0o400 accepted, 0o620/0o602/0o666 rejected as AuthError), empty schemaVersion rejection naming SchemaVersionV1, unsupported version rejection, unknown-key rejection via yaml.UnmarshalStrict, and a Save round-trip producing 0o600. **Postmortem lesson:** the roadmap's "missing schemaVersion" bullet was a dead branch (DefaultConfig() pre-populates v1, so the missing-field error path requires `schemaVersion: ""` explicitly) — testing surfaced the dead-code shape before a future refactor accidentally made it reachable.

- **`tst:fde34e0c:kubeconfig-env-validator-untested`** — done 2026-05-21 — PR #692, merge commit `1b42552`. Tier K major (trust-boundary-untested). validateKubeconfigEnv at `internal/cluster/k8s.go:78-97` is a trust-boundary validator (Lstat to refuse symlinks, $HOME or /etc prefix allowlist, sep-guarded HasPrefix to defeat the /etcd/foo spoof). No test locked any of the three invariants. Added k8s_test.go with seven subtests using t.Setenv("HOME") for deterministic prefix resolution: /dev/zero rejected, /proc/self/environ rejected, symlink rejected with "symlink" substring, $HOME/.kube/config accepted, traversal landing outside both prefixes rejected, missing path rejected with "inaccessible" substring, /etcd/foo spoof rejected. **Postmortem lesson:** the roadmap Fix described `../../../etc/passwd` as "canonicalised then rejected" but the current validator would *accept* /etc/passwd via the /etc prefix — the test surfaced this contradiction, which would have been impossible to find by reading the code alone since the test's job is to exercise the actual behaviour.

- **`tst:19a715fd:secretstore-manifest-untested`** — done 2026-05-21 — PR #693, merge commit `acbf40c`. Tier K major (cred-path-untested). secretManifestFromFile at `internal/addon/catalog/secretstore/secretstore.go:284-307` reads plaintext token/credentials files and base64-builds Opaque Secrets via `addon.BuildOpaqueSecret`; resolveSecretsDir handles absolute/relative/empty SecretsDir against env.ProjectRoot. None of the resolution branches or the manifest builder were tested. Added secretstore_test.go covering all three resolveSecretsDir branches, a traversal anchor (SecretsDir="../etc" records current permissive behaviour for future allowlist work), and TestSecretManifestFromFile asserting YAML kind=Secret, apiVersion=v1, type=Opaque, base64-decoded data round-trip (newline-trimmed), and no plaintext leak in the manifest text. **Postmortem lesson:** when an addon helper builds a Kubernetes Secret manifest containing plaintext-derived bytes, the test must assert the plaintext does NOT appear unencoded in the manifest — otherwise a future refactor that drops the base64 step ships secrets in cleartext to the cluster.

- **`tst:29293401:configure-haproxy-untested`** — done 2026-05-21 — PR #694, merge commit `887180d`. Tier K major (destructive-untested). ConfigureHAProxy at `setup/haproxy.go:91-144` writes /etc/haproxy/haproxy.cfg with backup + rollback semantics — attemptHAProxyRollback alone was tested but the surrounding flow (backup creation, AtomicWriteString of new config, haproxy -c validation gate, enable+restart, rollback-on-validation-fail, rollback-on-restart-fail) was untested. Promoted haproxyConfigPath/haproxyBackupPath from const to var and added `enableAndRestartHAProxyFn` as a package-level seam so tests can substitute a no-op without hitting systemctl. Added haproxy_test.go with happy-path-no-backup, happy-path-backup-created, and no-backup-skips-rollback tests using a fake haproxy script via t.Setenv("PATH"). **Postmortem lesson:** a function that writes to /etc/* with rollback semantics needs three package-var seams (config path, backup path, service-control function) before it becomes testable — the lack of seams is the lack of testability, and tests force the right shape on the production code.

- **`tst:5013fea6:release-extract-auth-untested`** — done 2026-05-21 — PR #695, merge commit `37856a1`. Tier K major (trust-boundary-untested). bootstrapOC at `setup/release_extract.go:97-144` fetches+verifies an oc tarball against a compile-time SHA-256 (supply-chain trust anchor); extractReleaseImage routes registry errors through isAuthError keyword matching for auth-vs-cluster taxonomy. Neither authMarkers' keyword set nor the bootstrap-checksum-mismatch wrap was tested. Promoted bootstrapOCChecksum from const to var and added bootstrapOCBaseURL as a package-level seam so httptest can stub the endpoint without compromising the runtime trust anchor. Added release_extract_test.go with table-driven isAuthError tests (5 positive markers + 3 negative), TestBootstrapOC_cachedSkipsFetch (pointer at unroutable host to prove caching skips the network), and TestBootstrapOC_checksumMismatch locking the *errtypes.NetworkError wrap. **Postmortem lesson:** const-to-var demotion for testability is acceptable only when the runtime contract is preserved (the production value initialises identically to the const; tests restore via t.Cleanup) — anyone reading the code should see the var as "fixed at package init, overridable in test scope" via a doc comment that names the constraint.

- **`err:4c092fce:terraform-execerror-stderr-latent-leak`** — done 2026-05-23 — PR #721, merge commit `be4bc21`. Tier K major (redaction-in-error — seam→audit-observability). `executor.ExitError.Error()` at `internal/executor/executor.go:206-208` embedded raw `strings.TrimSpace(e.Stderr)` verbatim; `Redacted()` omitted Stderr for slog but any caller calling `err.Error()` or `fmt.Sprintf("%v", err)` bypassed the structured-attr redaction. Terraform stderr can carry PROXMOX_VE_PASSWORD/PROXMOX_VE_API_TOKEN echoed during provider plugin errors, so the latent leak was real even if no current call site exposed it. Routed the embedded Stderr through `logutil.RedactableStderr(...).Redacted()` (200+200 byte head/tail truncation) so the formatted message is bounded at <=400 bytes regardless of subprocess output. Added TestExitError_ErrorTruncatesLongStderr and TestExitError_ErrorShortStderrPassesThrough to lock the contract. **Postmortem lesson:** when a type has a `Redacted()` method for slog but its `Error()` returns raw fields, the type's safety story depends on caller discipline — folding the same truncation into `Error()` is the type-level fix, not the call-site fix; "structured logging is safe" is not a guarantee about every caller's stringification habits.

- **`tst:a6e38cc7:sshpin-verify-shellout-untested`** — done 2026-05-23 — PR #722, merge commit `4bcfb1b`. Tier K major (trust-boundary-untested). `sshpin.Verify` (`internal/sshpin/sshpin.go:35-51`) owned the trust-on-first-use boundary for the Proxmox SSH connection: it shelled out to `ssh-keyscan -T 5` then delegated to parseAndMatch. parseAndMatch was tested; Verify's subprocess composition, error-prefix wrap, and ctx-cancellation path were not. A regression dropping `-T 5` would silently hang `okdctl destroy` against an unreachable Proxmox host. Added sshpin_verify_test.go with installFakeKeyscan(t, script) installing a POSIX sh fake at a tempdir PATH; three tests cover happy-path matching the existing fixtureKeyscanLine, non-zero exit wrapped with the "ssh-keyscan <host>:" prefix, and ctx-deadline cancellation against a `sleep 300` fake. **Postmortem lesson:** the original CtxDeadline assertion checked `errors.Is(err, context.DeadlineExceeded)` — but `system.OutputCaptured` wraps `cmd.Output`'s "signal: terminated" which does not preserve the ctx error in its Unwrap chain. The CI green test asserts on (a) the wrap prefix and (b) elapsed time well below the 300s fake sleep, which locks the actual cancellation contract without depending on chain semantics the executor does not guarantee.

- **`tst:ab9b764a:install-config-pull-secret-zeroize-untested`** — done 2026-05-23 — PR #723, merge commit `45078cb`. Tier K major (cred-path-untested). `GenerateInstallConfig` (`internal/distribution/okd/setup/ignition.go:39-98`) read the pull-secret into a `[]byte` and `defer system.ZeroBytes(pullSecret)`. The defer invariant was undocumented in tests — a future regression to `defer pullSecret = nil` (no clear) would leave the secret reachable until GC, and the pull-secret is the upstream registry credential embedded into install-config.yaml. Introduced `zeroBytesFn` package var as a minimal test seam (`var zeroBytesFn = system.ZeroBytes`), changed the defer to `defer zeroBytesFn(pullSecret)`, and added TestGenerateInstallConfig_PullSecretInFileAndZeroed that overrides the seam with a spy capturing the slice header, asserts the on-disk install-config.yaml contains the raw pull-secret literal (positive control before zeroing fires), and after return verifies every captured byte is 0x00. **Postmortem lesson:** when a `defer fn(buf)` carries a security invariant, the seam-via-package-var pattern locks the invariant against future refactors at the cost of one mutable var; a doc comment naming it as a test-only override (with "production code must not change it") keeps the contract explicit.

- **`sec:fde34e0c:input-kubeconfig-not-resolved`** — done 2026-05-23 — PR #724, merge commit `e41c328`. Tier K minor (input-validation). `validateKubeconfigEnv` (`internal/cluster/k8s.go:78-97`) rejected symlinks and enforced the $HOME or /etc prefix allowlist but never checked file mode bits. A world-readable kubeconfig in the env path was silently accepted, which let any local user read the cluster admin bearer token. Sibling `.env` file path was already gated at 0o077 (`internal/credentials/envfile.go:147-153`); kubeconfig holds equivalent credential material and warranted the same gate. Added `perm&0o077 != 0` check returning `&errtypes.AuthError{Msg, Path, Err: os.ErrPermission}` and three sub-tests (perm_0600_accepted, perm_0644_rejected_with_auth_error, perm_0620_rejected_with_auth_error) using `os.Chmod` after `os.WriteFile` to bypass the test process umask. **Postmortem lesson:** `os.WriteFile(path, data, perm)` is filtered through the process umask before reaching the file mode bits; tests asserting "file at perm X is rejected" need an explicit `os.Chmod` to restore the requested perm, or the umask silently strips the bits the test was trying to assert against.

- **`sec:eb479d86:cred-in-argv`** — done 2026-05-23 — PR #725, merge commit `5622dfb`. Tier K suggestion (shell-injection). `remoteISO256` at `internal/distribution/okd/setup/upload.go:23-40` validated `filename` via phase.ValidateRemoteFilename but passed `remotePath + "/" + filename` straight into SSHRunArgv as a single argv. The single live caller passes the `phase.DefaultProxmoxISODir` constant, but the helper boundary didn't enforce the contract — any future caller forwarding an operator-supplied remotePath would route an unvalidated string into the remote sha256sum target. Exported `validateISODir` → `ValidateISODir` from the phase package (matching the existing `ValidateRemoteFilename` export shape) and added a boundary call in remoteISO256. **Postmortem lesson:** when a function takes two args that flow into argv and only one is validated, the validation contract is implicit — boundary-level validation costs one line and survives future caller additions where docstring-level contracts decay; "the only caller passes a constant" is the trap before someone refactors the call site without re-deriving the assumption.

- **`sub:97cb8adf:unbounded-output`** — done 2026-05-23 — PR #726, merge commit `791c85e`. Tier K minor (io-handling). `OutputCaptured` at `internal/system/exec.go:72-86` used `cmd.Output()` which buffered the full subprocess stdout into a single `[]byte` with no cap. Callers include ssh-keyscan, gpg --import-options show-only, terraform output -json, dpkg -l, nmcli, ip addr/route, ufw status — any of which can return multi-MB output (especially in incident states). Replaced `cmd.Output()` with `cmd.StdoutPipe()` + `io.LimitReader(pipe, outputCapturedMaxBytes+1)` + `io.ReadAll`; overflow returns a `*SubprocessError` with "output exceeded N bytes" before the process Wait completes. Cap set to 4 MiB (4*1024*1024). **Postmortem lesson:** `cmd.Output()` is the convenient case of `Start+Wait+ReadAll(StdoutPipe)` without the LimitReader — code paths that may interact with unfriendly external tools should never use it; the canonical executor.Executor's ring-buffered Stdout (200 lines × 64 KiB) exists for exactly this reason, and the parallel wrapper inherited the convenience-trap. The LimitReader+1 idiom is the standard overflow-detection sentinel — reading `cap+1` bytes lets you distinguish "exactly cap" from "exceeded" with one syscall path.

- **`iac:e076e43c:sh-no-version-tag-validation`** — done 2026-05-23 — PR #727, merge commit `83bc57e`. Tier K minor (install-sh-fail-closed). `scripts/install.sh` resolved VERSION from the GitHub API tag_name (or user env) and dropped it straight into ARCHIVE_NAME + URL interpolations without shape validation. A tampered tag_name like `v1.0.0/../../evil` would alter the archive path that curl fetched; the cosign + sha256 layers caught the final tamper but the failure mode surfaced as a confusing "no checksum found" downstream rather than a clear "unexpected version tag" upfront. Added a single chokepoint regex after both API-resolution and user-supplied branches converge: `[[ "$VERSION" =~ ^v[0-9]+\.[0-9]+\.[0-9]+(-[A-Za-z0-9.-]+)?$ ]] || die "unexpected version tag: $VERSION"`. **Postmortem lesson:** fail-closed integrity layers (cosign + sha256) are necessary but the operator-facing diagnostic should fire at the first shape violation, not at the deepest verification step — a regex one-liner above the URL interpolation gives a clean error message and pre-empts every downstream confusion the deeper layers would produce on a malformed tag.

- **`err:d7ce9d16:errors-join-bare-fmt-errorf`** — done 2026-05-23 — PR #728, merge commit `00058ab`. Tier K minor (wrapping). `validateAndRestartDnsmasq` (`internal/distribution/okd/dns/dns.go:258-264`) wrapped its config-validation failure as `errors.Join(fmt.Errorf("dnsmasq config validation failed — previous config restored"), err)` — treating the human label as a peer error rather than wrapping context. The adjacent restart branch at L266-268 already used `fmt.Errorf("... — previous config restored: %w", err)`. errors.Is on the underlying err still walked through Join's Unwrap []error, so the chain semantics were preserved, but the idiom split was the smell. CLAUDE.md is explicit: "errors.Join is for joining peer errors; fmt.Errorf with %w is for single-cause wrapping." Replaced with `fmt.Errorf("dnsmasq config validation failed — previous config restored: %w", err)`. The `restore()` closure discards its CopyFile error, so there is exactly one cause to wrap. Removed the now-unused `"errors"` import. **Postmortem lesson:** errors.Join is correct only when there are >=2 peer errors with no parent/child relationship — when a function returns a single cause with a human label, %w is the canonical idiom, and the two patterns are not interchangeable even if errors.Is happens to work through both.

- **`err:632c9087:update-ingress-bare-fmt-errorf`** — done 2026-05-23 — PR #729, merge commit `7cc132a`. Tier K minor (sentinel-vs-typed). Six bare `fmt.Errorf` sites in `handleHostNetworkConversion`, `convertToLoadBalancer`, and `waitForServiceLB` (`internal/distribution/okd/postinstall/update_ingress.go:336-664`) returned errors that reached the right exit code (4) only because `orchestrator.classifyStepErr` wrapped non-typed errors in `*errtypes.ClusterError` as a fallback. But `attemptRollback` (L586) calls these helpers outside the orchestrator path — there the typed wrap never fires, so direct callers saw bare errors and the exit-code contract was silently orchestrator-dependent. Wrapped each site as `&errtypes.ClusterError{Msg: ..., Err: err}` matching the uniform sibling pattern at L68-L275; classifyStepErr's errors.As check passes through already-typed errors so no double-wrap. **Postmortem lesson:** when an orchestrator-level safety net (classifyStepErr) silently fixes 14+ bare errors across the codebase, the helpers' contracts are silently coupled to the orchestrator — any code path that bypasses the orchestrator (rollback, direct test, future re-entry) breaks the exit-code contract without any compile-time signal.

- **`api:ddf885f4:opt-name-manager-option`** — done 2026-05-23 — PR #730, merge commit `b4d4a6f`. Tier K minor (option-consistency). `addon.ManagerOption` (`internal/addon/manager.go:26-43`) used the qualifier prefix while every sibling functional-options type in the repo (cluster.Option, executor.Option, terraform.Option, proxmox.Option, firewall.Option, download.Option, phase.BasePhaseOption, okd.ProvisionerOption) uses the unqualified `Option` name. Call sites read as `addon.ManagerOption` where `addon.Option` suffices — the `Manager` token duplicates the package. Renamed ManagerOption → Option in manager.go (one type declaration + four function return types + one variadic parameter). External callers at `internal/cli/addon.go:263` and `internal/distribution/okd/postinstall/phase.go:77` use `addon.WithExecutor/WithLogger/WithProjectRoot` by name and never spell the type out, so they required no changes. Follow-up commit removed a gofumpt-flagged double blank line introduced by the initial Edit. **Postmortem lesson:** repo-wide naming conventions for type-level patterns (functional options, error types, phase Options structs) should be enforced via golangci-lint's `revive` or a custom analyzer — relying on PR review to catch one-off divergences misses the case where the divergent name was the original and the convention emerged later.

- **`iac:18a795d5:hcl-iso-list-no-empty-check`** — done 2026-05-23 — PR #731, merge commit `0a2f792`. Tier K suggestion (hcl-destroy-ordering). Master and worker lifecycle preconditions at `infrastructure/terraform/modules/proxmox-okd/main.tf:254/380` checked `length(var.master_isos) >= var.master_count` but not that each element was non-empty. A list with one empty entry passed the length gate then failed at apply with a cryptic provider error rather than at plan with a useful precondition message. Bootstrap was already covered by `var.bootstrap_iso != ""` (L123) — a scalar. Strengthened both preconditions to `length(...) >= count && alltrue([for iso in slice(var.X_isos, 0, var.X_count) : iso != ""])`. The slice bounds the check to the entries that will actually be used (a 5-entry list with empty entries 4-5 stays valid for a 3-node count). **Postmortem lesson:** Terraform preconditions catch what variable validations cannot — a `length` check on a list does not imply non-empty elements; the `alltrue + slice + for` idiom is the canonical "every used element satisfies P" shape, and putting it in the lifecycle precondition (not the variable block) keeps the error scoped to the resource being created at plan time.

- **`ux:fd2125dd:help-long-missing`** — done 2026-05-23 — PR #732, merge commit `c8eb42b`. Tier K minor (help-text). `addonVerifyCmd` (`internal/cli/addon.go:95-100`) had Short and Example but no Long description; sibling addon subcommands (list, install, uninstall) all carried Long blocks explaining failure modes, dependencies, and the exit-code contract. Generated docs at `docs/cli/okdctl_addon_verify.md` rendered with no Synopsis section, making the verify reference read as an afterthought via `--help` and the markdown docs. Added a Long describing the live-cluster probe shape, per-addon pass/fail output, and the non-zero exit on probe or config-load failure, matching the doc voice of `addonListCmd`. Regenerated the CLI reference via `go run -tags docs ./cmd/okdctl-gen-docs`. **Postmortem lesson:** cobra Long is the field that survives into both `--help` output AND the markdown docs; treating Short as sufficient for "simple" verbs is the gap that leaves the operator without the failure-mode explanation that every sibling carries. CI's docs-go check would have failed the PR if the generated file fell out of sync — that's the wire that keeps docs honest.

- **`ux:aa84670c:help-long-missing`** — done 2026-05-23 — PR #733, merge commit `b9f5d3b`. Tier K minor (help-text). `versionCmd` (`internal/cli/root.go:238-246`) had only Short — no Long, no Example, no --output=json. Sibling read-only commands (status, releases, doctor) all expose Long+Example and --output=json. Tools consuming `okdctl version` programmatically (CI version pinning, scripted comparisons) had to parse free-form text. Added Long, Example (`okdctl version` and `okdctl version --output json | jq .version`), and wired `--output text|json` via the existing flagOutput/flagOutputShort/validateFormat/quietForJSON helpers. Introduced `versionOutput` struct with json-tagged fields {version, git_commit, build_date, go_version, platform} emitted via the existing writeJSON helper. Documented the shape in `docs/cli/json-schema.md`. Regenerated CLI reference. **Postmortem lesson:** when every read-only sibling exposes a stable JSON form via shared helpers (validateFormat, writeJSON, quietForJSON), adding the same shape to a holdout command is a 30-line patch — the helpers are the contract; the missing flag is the divergence; the json-schema.md update is the contract entry.

- **`ux:e45c2239:stderr-stdout-mixed`** — done 2026-05-23 — PR #734, merge commit `b33ba13`. Tier K minor (streams). `preflight()` (`cmd/okdctl/main.go:32-51`) called `tui.Warn` before `cli.Execute` ran `configureLogging` — so the OKDCTL_BIN_DIR validation warning fired against whatever the package-init configured (text-format), not against the user's --log-format=json choice. JSON consumers piping 2>&1 saw text-format warnings interleaved with JSON log lines. Added `preflightWarns []func()` package var in `internal/cli/root.go` plus exported `DeferWarn(fn func())` appender; PersistentPreRunE drains the slice after configureLogging completes and before ensureRoot runs, then nils the slice so re-entry (sudo re-exec) does not double-emit. main.preflight wraps each tui.Warn in a closure passed to cli.DeferWarn. **Postmortem lesson:** any code that runs before cobra's PersistentPreRunE cannot rely on the logger configuration the user requested via flags — the closure-deferred pattern (capture log args at the call site, fire after the formatter is set) is the clean separation; alternatives that move SetDefault earlier lose the format choice because --log-format is parsed by cobra itself.

- **`obs:15ba17da:key-inconsistent-casing`** — done 2026-05-23 — PR #735, merge commit `7e9ad5b`. Tier K suggestion (field-stability). Destroy's summary step at `internal/distribution/okd/destroy/steps.go:180-184` logged the failed-steps list under key `steps` (plural) and the skipped-steps list under `skipped` — but the orchestrator's per-step grep contract uses `step` (singular) consistently (orchestrator.go:L171). A consumer grepping for `step` would not match these summary entries even though they carried conceptually-related data. Renamed both keys to `failed_steps` and `skipped_steps`. Replaced `strings.Join(list, ", ")` with passing the `[]string` directly to slog (KindAny) so the consumer gets an array-typed value (rendered by slog as `[item1 item2 ...]` via fmt.Sprint) instead of a comma-joined string. Removed the now-unused `"strings"` import; updated steps_test.go to look up the new keys. **Postmortem lesson:** slog key naming is the grep contract — once `step` exists for the singular case anywhere, `steps` for the plural-batch case is a silent collision; the fix is always to pick a fully-qualified plural noun (failed_steps, skipped_steps), and passing the slice directly preserves the array shape for downstream consumers.

- **`obs:5013fea6:level-error-not-user-visible`** — done 2026-05-23 — PR #736, merge commit `a7ab9dd`. Tier K minor (level-discipline). `extractReleaseImage` (`internal/distribution/okd/setup/release_extract.go:126-136`) called `p.Log.Error("tools: oc adm release extract failed", ...)` before returning a typed `*errtypes.AuthError` or `*errtypes.ClusterError`. `cli/root.go::execute()` then unconditionally logged the same error a second time at the top of the stack via `tui.Error("command failed", "err", err)`. The local Error double-logged the failure event and front-loaded it before the caller's classification picked the right exit code. Demoted the local log to `p.Log.Warn` — the caller's Error log at the top of the stack remains the sole user-visible Error event; the per-step trace (with ref + stderr attrs) stays one level below for diagnostic context. **Postmortem lesson:** Error level in a per-step trace is a UI claim — "this is the thing the operator should look at" — but if the caller is also going to emit an Error at the top of the stack on the same failure, the per-step log should be Warn; the level discipline rule is "exactly one Error per terminal failure event".

- **`doc:b3356305:readme-flag-ghost`** — done 2026-05-23 — PR #737, merge commit `f1a7f93`. Tier K minor (readme-drift). README.md L240 referenced `okdctl postinstall` which does not exist as a cobra subcommand — `okdctl deploy` runs the postinstall phase internally as its final step. Operators following the README's security mitigation bullet would run `okdctl postinstall` and get a "unknown command" error. Replaced the bullet to name `okdctl deploy` (which actually performs the documented sequence), preserving the surrounding ignition-file-cleanup guidance. Verified against `grep -n 'Use:' internal/cli/*.go` that `deploy` and `cleanup` both exist as cobra Use strings while `postinstall` does not. **Postmortem lesson:** every command name in README that does not map to an entry in the cobra command tree is a tripwire — a one-line `make doc-check` script that greps every backtick-quoted `okdctl X` token against the cobra Use list would catch this class automatically.

- **`doc:4ac0d12f:line-citation-drift`** — done 2026-05-23 — PR #738, merge commit `1ec7c2a`. Tier K minor (readme-drift). `docs/addons/secretstore.md:34-35` cited `secretstore.go:19-30` as the location of the setting-key constants, but those constants moved to `internal/addon/catalog/secretstore/secretstore.go:25-39` (with an unrelated defaultSecretsDir/defaultNamespace block at L18-22). Hard-coded line ranges rot every time the file is touched. `docs/addons/flux.md:31` already used symbol-name citations without line numbers as the stable shape. Replaced the line-range citation with symbol-name references (`Setting*` constants and `DefaultSettings`) plus the full package-relative path. **Postmortem lesson:** "see file.go:19-30" in docs is an invitation for drift — symbol names stay stable across edits and survive renames the way numeric ranges do not; the docs/addons/flux.md citation style is the canonical shape for this codebase, and the difference between the two files (one uses symbols, one used numbers) was the audit-finding seed.

- **`state:bdf5a873:work-dir-cleanup-no-resume`** — done 2026-05-27 — PR #769, merge commit `42253ac`. Tier K suggestion (crash-recoverability). `cleanup.WorkDirectory` (`internal/distribution/okd/cleanup/artifacts.go:67-69`) had no audit anchor for its safety baseline: a SIGKILL mid-WorkDir-removal leaves a partially-stripped tree, and the audit confirmed this is benign because every subsequent Full-sequence cleanup step (webServer/haproxy/dnsmasq/terraform/packages/ignition-certs) references paths OUTSIDE workDir (HTTPServerRoot, /etc/haproxy, /etc/dnsmasq.d/, infrastructure/terraform/, binDir, ProjectRoot/certs). Without the anchor, a future audit pass would re-flag the same concern and re-derive the proof. Added a four-line godoc note citing `state:bdf5a873` and enumerating the post-WorkDir steps so the audit-positive baseline is greppable. **Postmortem lesson:** an audit-positive baseline is only durable if it survives in the code, not in the audit JSONL — a comment that names the audit ID + the proof becomes the regression guard for the next reviewer who reads the WorkDirectory godoc.

- **`api:3e02f6b8:export-no-caller-vmstate`** — done 2026-05-27 — PR #770, merge commit `2b67a79`. Tier K suggestion (exported-surface). VMState's const block (`internal/distribution/okd/phase/vmstate.go:10-17`) had a bare label comment "VM lifecycle state values. String literals match the Proxmox API." but no scaffolding anchor — and four of the five constants (StateStopped/StateCreating/StateDeleting/StateUnknown) have no in-repo caller, only StateRunning is consumed by proxmox.Provider. `deadcode` flags this surface every run. Replaced the label with a three-line Scaffolding citation naming `api:3e02f6b8`, the future-status-path rationale, and the sole-consumer fact. **Postmortem lesson:** when an enum's call graph today only exercises one variant but the matrix is exhaustive by design, the godoc must name both the future consumer and the current sole consumer — otherwise `deadcode`'s false-positive sticks because the reader has no way to distinguish "scaffolding" from "dead branch".

- **`api:dd75bdeb:export-no-caller-phasecontext`** — done 2026-05-27 — PR #771, merge commit `fdf805b`. Tier K suggestion (exported-surface). `distribution.PhaseContext[T]` (`internal/distribution/context.go:5-11`) already had a strong godoc naming the scaffolding intent and the dependent roadmap IDs (state:4f69fc9d, state:262af6e4) but did not cite the api-design audit ID that flagged the type. The fix was a one-token inline insertion — `api:dd75bdeb;` — into the existing roadmap-ID parenthetical, keeping line count unchanged. **Postmortem lesson:** when a scaffolding type already carries roadmap-ID anchors for downstream consumers, the inline-into-existing-paren pattern is the cheapest audit citation; replacing the whole comment block invites whitespace/wrap churn that the diff doesn't need.

- **`state:fb54208a:postinstall-no-rollback-recurring`** — done 2026-05-27 — PR #772, merge commit `19765b0`. Tier K minor (crash-recoverability — related: `state:632c9087:update-ingress-haproxy-rollback-missing`). `postinstallSteps` (`internal/distribution/okd/postinstall/steps.go:43-56`) defines StepCleanupBootstrap which destroys the bootstrap VM via targeted apply. If StepVerifyKubeVIP (the next NonFatal step) fails, StepDeployProductionDNS's SkipWhen reads `!KubeVIPVerified` and skips, leaving DNS pointed at the now-gone bootstrap. This recurring finding (also tracked as `state:fb54208a:postinstall-no-rollback-path`) needed an in-code anchor naming the gap because the failure mode is the ordering invariant between three steps — invisible from any single step's body. Added a four-line comment above StepCleanupBootstrap citing the audit ID, naming the gap, and pointing to the preferred future fix (extend update-ingress to detect bootstrap-DNS + bootstrap-VM-gone). **Postmortem lesson:** when an ordering invariant spans three steps and the failure mode only materialises in the partial-success case, the comment belongs above the FIRST mutating step in the sequence — not on each step individually — because the reader needs the gap explained before they read the bodies that participate in it.

- **`api:9ce5434c:export-test-only-pollinterval`** — done 2026-05-27 — PR #773, merge commit `7858bca`. Tier K suggestion (exported-surface). `BasePhase.OcPollOutputInterval` (`internal/distribution/okd/phase/kubectl.go:48-52`) was exported solely as a test-injection seam — production code uses OcPollOutput which forwards interval=0. The existing docstring named the test-only constraint but not the audit ID, so `deadcode` re-flagged the surface every run. Appended a two-line scaffolding tail citing `api:9ce5434c` with explicit "do not delete without a replacement test-injection path" guidance. **Postmortem lesson:** test-injection seams need explicit "do not delete" with a replacement-condition because `deadcode` cannot distinguish "test-only export" from "abandoned API" — the seam contract has to live in the godoc, not in a test-package comment that the production reader never sees.

- **`api:cfcdee2d:export-no-caller-timeout-download`** — done 2026-05-27 — PR #774, merge commit `1ef0976`. Tier K suggestion (exported-surface — seam→`audit-modernization`). `httputil.TimeoutDownload` (`internal/httputil/httputil.go:18-22`) is the 5-minute tier of a symmetric latency trio (TimeoutShort/TimeoutMedium/TimeoutDownload) but no caller routes through it today — only Short and Medium are consumed. `deadcode` keeps flagging it; deleting would break the tier shape and the next file-download caller would have to reintroduce a 5-minute constant from scratch. Replaced the bare group label with a four-line Scaffolding block citing `api:cfcdee2d` and explaining the tier rationale. **Postmortem lesson:** when a const tier has one unused member but the trio is the API shape, the comment belongs on the GROUP COMMENT (before the `const (` line), not as a trailing remark on the unused member — readers scanning the package see "tier" before they see "unused", which is the right framing for the reader's mental model.

- **`err:a4001485:errtypes-no-recoverable-type`** — done 2026-05-27 — PR #775, merge commit `0573e3f`. Tier K suggestion (domain-vocabulary). The errtypes package doc (`internal/errtypes/errtypes.go:5-10`) already documented the intentional TransientError deferral and cited roadmap consumer `err:9f8e7d6c` — but did not list the three sites today that implement ad-hoc retry classification (`infrastructure/proxmox/proxmox.go::initIsRetryable`, `addon/helpers.go::addonIsRetryable`, `download/retry.go::isRetryable`). Without the site enumeration, the future TransientError implementer has no map of where to consolidate. Extended the deferral sentence with a "see audit err:a4001485" reference and the three call-site paths. **Postmortem lesson:** when a package doc documents a deferred type that will consolidate N ad-hoc implementations, the deferral note must enumerate the N sites by file:symbol — otherwise the consolidation work starts with a `grep -r` rather than the doc, and one site silently misses the rename.

- **`err:b804b2ec:bootstrap-tfvars-classification`** — done 2026-05-27 — PR #776, merge commit `d149193`. Tier K suggestion (sentinel-vs-typed). `CleanupBootstrap` (`internal/distribution/okd/postinstall/bootstrap.go:77-80`) writes `bootstrap-state.auto.tfvars.json` and wraps a failure as `*errtypes.ClusterError` — correct because the write is part of the cluster lifecycle (after a successful bootstrap-vm destroy) and not user-config parsing. The file's `.tfvars` suffix invites the misreading "this is config → use ConfigError"; the audit flagged the classification for documentation, not change. Added a two-line inline comment above the return citing `err:b804b2ec` and naming the rule: state-write during cluster lifecycle → ClusterError (exit 4); the file is managed by okdctl, not the user. **Postmortem lesson:** the file extension is not the classification signal — provenance is. Any file the operator never edits routes failures through cluster-lifecycle error types regardless of its `.yaml`/`.json`/`.tfvars` suffix, and that rule deserves an inline comment at every site where a future reviewer might "fix" it the wrong way.

- **`state:eb479d86:iso-upload-already-done-sha256`** — done 2026-05-27 — PR #777, merge commit `29c8737`. Tier K suggestion (phase-idempotency). `isoUploadAlreadyDone` (`internal/distribution/okd/setup/upload.go:179-181`) returns `(false, nil)` conservatively on any SSH error or absent Proxmox config — the existing `nolint:nilerr` comment named the policy at the call site but the function godoc didn't cite the audit confirmation. Without the anchor, a future audit re-derives the proof from scratch. Appended a two-line tail citing `state:eb479d86` and naming the invariant: conservative-not-done lets Exec surface the real failure rather than silent skip. **Postmortem lesson:** when a `nilerr` discards an error class deliberately, the policy comment belongs in BOTH places — at the `nolint` site (which the linter reads) and in the function godoc (which the audit reader reads); they serve different audiences and the linter directive doesn't survive into godoc renders.

- **`api:588ce79e:export-color-theme-enum`** — done 2026-05-27 — PR #778, merge commit `f4ca994`. Tier K suggestion (exported-surface). `tui.ColorTheme` + `ThemeDefault`/`ThemeHighContrast` (`internal/tui/colors.go:9-16`) are exported but the only consumer is package-private `setTheme()`. The enum exists for a future `okdctl theme` CLI verb. Replaced the bare "Built-in color themes." group label with a three-line scaffolding note citing `api:588ce79e` and naming the future caller. **Postmortem lesson:** exported names whose only current caller is package-private read as "leaked accidentally"; the godoc needs to name the planned external caller explicitly to flip that reading to "intentional API surface waiting for its verb".

- **`smell:cf43073b:enum-ad-hoc`** — done 2026-05-27 — PR #779, merge commit `53f5f2a`. Tier K suggestion (magic-strings). `DistributionType` and `ProviderType` (`internal/config/types.go:1-32`) are typed string enums with exactly one value each — DistributionOKD, ProviderProxmox — and SupportedDistributions/SupportedProviders return one-element slices. Per roadmap Product philosophy §1 multi-provider/multi-distribution is explicitly SKIPPED, so the single-variant shape is API stability scaffolding, not extensibility scaffolding for a never-coming second provider. Without an anchor, a code-smell auditor reads the enums as premature abstraction. Added a three-line Scaffolding block to each type citing `smell:cf43073b` and pointing to roadmap §1. **Postmortem lesson:** when product policy SKIPS the future expansion that an enum's shape implies, the anchor must name the policy ID (roadmap §1) — otherwise the next reader assumes the enum is awaiting growth and the smell recurs.

- **`err:d6b325cb:proxmox-sentinel-ad-hoc`** — done 2026-05-27 — PR #780, merge commit `68df040`. Tier K suggestion (sentinel-vs-typed). `proxmox.ErrNotConnected` and `proxmox.ErrTerraformNotConfigured` (`internal/infrastructure/proxmox/types.go:9-13`) live in the proxmox package while the repo's stated error vocabulary lives in errtypes. The audit confirmed leave-as-is: package-local sentinels for package-internal preconditions are idiomatic Go (cf. `io.EOF`, `fs.ErrNotExist`), and every call site wraps them in `ConfigError` so `errors.As` classification reaches the correct exit code. Expanded the bare "Provider sentinel errors." comment to a four-line block citing `err:d6b325cb`, naming the idiomatic-Go precedent (io.EOF, fs.ErrNotExist), and noting the ConfigError-wrap contract. **Postmortem lesson:** "the repo's vocabulary lives in errtypes" is a convention, not a rule — the canonical exceptions (io.EOF-shaped sentinels for package-internal preconditions) deserve an anchor that names BOTH the precedent AND the contract that keeps the chain semantics correct, so the next audit pass doesn't re-open the placement question.

- **`api:bb4fb1a3:zero-value-csr`** — done 2026-05-27 — PR #781, merge commit `32b9d3b`. Tier K suggestion (zero-value-usability). `cluster.CSR` (`internal/cluster/types.go:3-9`) is a single-field struct (Name string) whose sole consumer is `PendingCSRs` returning `[]CSR`. The struct shape is API stability scaffolding for future fields (ExpiresAt, Signer) but the existing godoc already explained the "Pending bool would always be true" choice without naming the audit ID or the future-fields rationale. Appended a three-line Scaffolding tail citing `api:bb4fb1a3` and noting "do not collapse to []string". **Postmortem lesson:** a single-field struct passes the smell-test only when the godoc names the planned multi-field future explicitly — otherwise every code-smell pass re-flags the same indirection because the reader has no way to distinguish "API stability" from "premature box".

- **`smell:04f0e35f:abstraction-single-caller`** — done 2026-05-27 — PR #782, merge commit `33c10d3`. Tier K suggestion (helper-package-no-value). `system.ZeroBytes` (`internal/system/zeroize.go:3-9`) is a one-line wrapper around stdlib `clear(b)` with one production caller (`GenerateInstallConfig` for pull-secret). The existing godoc named ProxmoxCredentials.Zeroize as the method-bound equivalent but didn't explicitly anchor the symmetric-vocabulary intent. Appended a three-line tail citing `smell:04f0e35f` and noting the canonical-byte-buffer-complement role so future credential-zeroize sites adopt the same vocabulary rather than inlining `clear()`. **Postmortem lesson:** a wrapper that adds no behavioral gain over stdlib is justified ONLY by the vocabulary it carries — the godoc must name the sibling site whose vocabulary it mirrors, because that is the entire reason to prefer the wrapper at every future call site.

- **`api:7f2bf677:pkg-pvesh-in-phase`** — done 2026-05-27 — PR #783, merge commit `43965e7`. Tier K minor (package-boundary). `phase/pvesh.go` (`internal/distribution/okd/phase/pvesh.go:25-34`) contains Proxmox-specific subprocess primitives (PveshRun) — they landed in `phase/` to break the proxmox→phase import cycle (proxmox imports phase for VMState/NodeRole, so pvesh helpers cannot live in proxmox without cycling back). The PveshRun godoc named the validation contract but not the break-cycle rationale. Added a four-line tail citing `api:7f2bf677`, naming the import-cycle constraint, identifying infrastructure/proxmox as the sole direct consumer, and pointing to the long-term direction (if iso_cleanup moves out of phase/, pvesh follows). **Postmortem lesson:** when a package-boundary compromise exists because of an import cycle, the comment must name the cycle direction (A imports B because B needs T from A) — readers cannot derive the cycle from the current layout alone, and the cycle is the only reason the unnatural placement persists.

- **`sec:48688e63:input-cidr-not-parsed`** — done 2026-05-27 — PR #741, merge commit `c342ef1`. Tier K suggestion (input-validation). `Provider.Connect` (`internal/infrastructure/proxmox/proxmox.go:131-145`) validated `Host` immediately but assigned `p.node` from `cfg.Provider.Proxmox.Node` without an explicit check — relying on the downstream `pveshRun`'s `validateProxmoxName` gate. A future caller that bypasses pveshRun and uses `p.node` directly in an ssh argv would pass an unvalidated name to the remote shell. Added `config.ValidateProxmoxNodeName` (re-using the existing `proxmoxNamePattern` `^[a-zA-Z][a-zA-Z0-9_-]*$`), called from Connect immediately after the host gate with the same `errtypes.ConfigError{Msg, Err}` wrap shape. **Postmortem lesson:** defense-in-depth at API entry points is cheap (one call, idempotent) and survives the future caller who finds a path around the downstream gate; relying on a single downstream check assumes every future call site reads that downstream code, which is not a maintainable assumption.

- **`ux:d31d1b9d:help-short-tone`** — done 2026-05-27 — PR #742, merge commit `fd18ad8`. Tier K minor (help-text). `describeCmd` (`internal/cli/status.go:38-42`) used colloquial "Drill into a specific node or addon" — out of voice with the rest of the surface where Shorts are clinical imperatives ("Print", "List", "Show", "Manage", "Verify"). Long was a meta-sentence about how to start, not what the verb does. Rewrote Short to "Show details for a cluster node or addon" matching `describeNodeCmd`/`describeAddonCmd` Short shape and tightened Long to a single descriptive sentence. Required a follow-up commit to regenerate `docs/cli/okdctl*.md` after CI's docs-go drift check flagged the stale generated reference. **Postmortem lesson:** any cobra Short/Long edit must be followed by `make docs` in the same commit — the generated CLI reference under `docs/cli/` is committed and verified by the `docs-go` job; missing the regeneration produces a single-file logical change but a two-commit PR with a transient CI red on the first push.

- **`dep:33ef32bf:claude-md-yaml-quad-outdated`** — done 2026-05-27 — PR #743, merge commit `c11aef6`. Tier K suggestion (duplicate-engine). CLAUDE.md §dependencies stated "Four YAML engines are in the dep tree" — but `go list -deps -test=false ./cmd/okdctl` confirms only TWO engines ship in the production binary: `sigs.k8s.io/yaml` v1.6.0 (direct) and `go.yaml.in/yaml/v2` v2.4.3 (indirect, pinned by k8s.io/apimachinery). `go.yaml.in/yaml/v3` is reachable only under `-tags docs` via `cmd/okdctl-gen-docs`; `gopkg.in/yaml.v3` is go.sum-only (absent from go.mod). Rewrote the tripwire bullet to reflect the two-engine production baseline with build-tool and go.sum-only classifications spelled out. **Postmortem lesson:** any CLAUDE.md claim about "what ships in the binary" must be verified by `go list -deps -test=false ./cmd/<name>` not by inspecting `go.mod`/`go.sum`; the dep graph diverges between build-tools (`-tags docs`), tests, and the production binary, and the binary-shipping subset is what the tripwire actually wants to guard.

- **`state:b5a79fda:deploy-state-marker-not-version-tagged`** — done 2026-05-27 — PR #744, merge commit `1178d4a`. Tier K suggestion (state-schema-evolution). `deployState` JSON (`internal/cli/deploystate.go:26-31`) had four fields (Phase, RunID, Timestamp, ClusterName) and no schema marker — a future schema bump would unmarshal old marker files with zero values for new fields, silently degrading the recovery announce path. Added `SchemaVersion string` field with `json:"schema_version"` tag + package constant `deployStateSchemaV1 = "v1"`; `writeDeployState` stamps it; `readDeployState` Warn-and-treats-as-absent on mismatch (the file is advisory per `announceDeployState`, not authoritative). Mirrors the `config.SchemaVersionV1` pattern. **Postmortem lesson:** schema versioning belongs on the FIRST commit of a JSON-on-disk format, not the n-th edit when the bump becomes painful; advisory data structures (where "missing" is a valid state) can pay back the migration cost with a Warn-and-skip path, but only if the version field exists from day one.

- **`obs:632c9087:any-on-volatile-type`** — done 2026-05-27 — PR #745, merge commit `dc595c1`. Tier K suggestion (field-stability). `update_ingress.go::UpdateIngress` (`internal/distribution/okd/postinstall/update_ingress.go:106-111`) logged the discovered IngressControllers via `strings.Join(descriptions, ", ")` against the "controllers" attr key — collapsing a `[]string` into a single comma-joined blob that downstream filters must substring-match instead of indexing. Replaced with `slog.Any("controllers", descriptions)` so the slog JSON handler renders a proper array; dropped the redundant "count" attr (slog rendering trivially reproduces len()). **Postmortem lesson:** slog attrs are typed; a slice-shaped value carries its shape into the JSON sink only if you pass the slice directly — `strings.Join` is a one-way collapse that costs nothing at the call site and costs everything at the grep/filter site downstream.

- **`ux:b3356305:help-no-example`** — done 2026-05-27 — PR #746, merge commit `0a46427`. Tier K minor (help-text — seam→audit-documentation). README.md §usage listed only five commands (deploy, destroy, update-ingress, doctor, --version) while `docs/cli/okdctl.md` SEE ALSO names 14 top-level commands. A reader scanning the README would not discover `okdctl status`, `okdctl debug-bundle`, `okdctl kubeconfig`, etc. Expanded §usage to mirror the SEE ALSO set with one-line cobra-Short descriptions; converted `okdctl --version` (flag) to `okdctl version` (subcommand). **Postmortem lesson:** the README usage block is the highest-traffic doc surface on the project — any subcommand added without backfilling §usage compounds drift; a one-line `make docs-check-readme` that diffs README §usage against the cobra command tree would catch this class automatically.

- **`sec:4c092fce:toctou-chmod`** — done 2026-05-27 — PR #747, merge commit `7ca7801`. Tier K minor (file-toctou). `SnapshotState` (`internal/infrastructure/terraform/terraform.go:395-414`) read `terraform.tfstate` with `os.ReadFile`, which follows symlinks. The state file contains Proxmox credentials marshalled by the bpg provider; a pre-sudo attacker could plant a symlink at the path and redirect the credential-bearing read into an attacker-chosen file (then copied to a 0o600 backup under workDir under the operator's uid). Replaced `os.ReadFile` with `os.Lstat` (refuse symlink) + `os.OpenFile(.., os.O_RDONLY|syscall.O_NOFOLLOW, 0)` + `io.ReadAll` + `Close`, mirroring `runlock.Acquire` and `ignition.readNoFollow` verbatim. **Postmortem lesson:** every credential-bearing read in a sudo-re-exec'd phase needs `O_NOFOLLOW` — TLS is the integrity gate for download paths, but for local-disk reads symlink-following IS the attack; the runlock pattern is the codebase's canonical answer and should be the default for any new `os.OpenFile` on operator-controllable paths.

- **`iac:e076e43c:sh-cosign-cert-identity-regex-loose`** — done 2026-05-27 — PR #748, merge commit `6431496`. Tier K minor (install-sh-integrity). `scripts/install.sh:156-157` matched any workflow path under the repo (`https://github\.com/qxtaiba/okdctl/`); a future stray signed workflow with valid Sigstore creds could satisfy the cosign cert-identity gate. Tightened to `^https://github\.com/qxtaiba/okdctl/\.github/workflows/release\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$` — anchored both ends, pins the goreleaser workflow path, and accepts optional pre-release suffix (`-rc.N`, etc.) since the release workflow trigger is `tags: v*`. The independent reviewer caught the missing pre-release support on the first round; required a follow-up commit. **Postmortem lesson:** when tightening a security regex, always check the trigger pattern of the workflow that produces the signed artefact — the workflow's tag pattern is the upper bound on what the cosign cert-identity will see, and an over-tight regex blocks legitimate pre-releases as effectively as a permissive one waves through attackers.

- **`obs:bbc23e42:handler-no-tty-switch`** — done 2026-05-27 — PR #749, merge commit `a7f2bab`. Tier K suggestion (handler-setup). `internal/tui/logger.go::var (` declared `stdoutLogger` and `stderrLogger`; only `stderrSlog` (wrapping `stderrLogger`) was routed through `logutil.RedactHandler`. The invariant was correct but undocumented — a future contributor wiring `slog.New(stdoutLogger.Load())` for a `--log-to-stdout` flag would silently bypass credential redaction. Added a four-line invariant comment above `stdoutLogger` that names the `slog.Default` exemption and the required `logutil.NewRedactHandler` wrap for any future stdout-targeted slog logger. **Postmortem lesson:** invariants protected by "we don't currently do X" become bugs the day someone does X — the protective comment must name BOTH the invariant AND the precise constructor that re-establishes it, so the future-implementer-of-X has the wrap function name in their hands before they reach for the wrong one.

- **`obs:fde34e0c:err-stringified`** — done 2026-05-27 — PR #750, merge commit `97ec3cd`. Tier K minor (field-stability). `internal/cluster/k8s.go:61` called `c.logger.Debug("ignoring $KUBECONFIG env value", "reason", err.Error())` — collapsing the error to a string before slog stored it as KindString, which `RedactHandler.redactAny` cannot inspect (no type switch path for plain strings; `*url.URL` userinfo and `Redacted() any` implementors silently bypass). Replaced with `"err", err` (KindAny dispatch). Also renamed the key from "reason" to "err" to match the codebase-wide idiom. **Postmortem lesson:** `err.Error()` is the canonical anti-pattern in any slog call site — it strips the type chain that RedactHandler's dispatch depends on, and the failure is silent (no compile error, no test failure) until a future error type carries a credential in its message; a one-line lint rule grepping for `err\.Error\(\)` in slog argument position would enforce it.

- **`mod:6fc3d91e:use-strings-fieldsseq`** — done 2026-05-27 — PR #751, merge commit `7cc559e`. Tier K minor (slices-maps). Two iterate-once sites — `internal/platform/platform.go:122` (detectFamily) and `internal/runlock/runlock.go:96` (crossHostHint) — used `for _, x := range strings.Fields(s)`, materialising a `[]string` slice that the loop discards after the first match. Replaced with `for x := range strings.FieldsSeq(s)` (Go 1.24 iterator form); no slice allocation, identical loop body. **Postmortem lesson:** when you see `strings.Fields(s)` in a range-over-and-discard pattern, `strings.FieldsSeq(s)` is always the right swap on Go 1.24+ — it's a free allocation reduction with zero behavioural change; same logic applies to `strings.SplitSeq` for split-and-iterate patterns.

- **`sec:35abd54e:cred-env-leak-to-child`** — done 2026-06-03 — PR #752, merge commit `f789fe0`. Tier K suggestion (credentials). `ProxmoxCredentials.Env()` (`internal/credentials/proxmox.go:146-156`) converts `[]byte` secret fields to Go strings via concatenation; the audit flagged the need for mechanical enforcement that every caller installs `defer ZeroizeEnv()` immediately. Building a custom linter was out-of-scope. Three call sites (cli/deploy.go, cli/destroy.go, cli/helpers.go) were verified clean; converted mechanical enforcement to reviewer enforcement by adding a "Known call sites" registry inline in the `Env()` doc comment and appending a reviewer-checklist sentence to CLAUDE.md §credentials-and-secrets pointing at the registry. **Postmortem lesson:** when "mechanical enforcement" requires building a linter that no one will maintain, an inline-doc registry + reviewer checklist captures the same intent at zero tooling cost — the trade-off is that a new caller must be added to the registry, which is exactly the diff a reviewer should see.

- **`err:97cb8adf:subprocess-stderr-tail-latent-leak`** — done 2026-06-03 — PR #753, merge commit `dc29125`. Tier K minor (redaction-in-error — seam→audit-observability). `SubprocessError.Error()` (`internal/system/exec.go:37-42`) concatenated raw `StderrTail` with no length cap; for `fmt.Errorf(..%w..)`-wrapped callers a large credential-bearing stderr would reach any string sink unbounded. Wrapped `e.StderrTail` in `logutil.RedactableStderr(...).Redacted()` inside `Error()` so the rendered string caps at <=400 bytes regardless of sink (mirrors `executor.ExitError.Error()` at executor.go:207). Added `TestSubprocessError_ErrorTruncatesLongStderr` covering the 500-byte → "[truncated]" path. **Postmortem lesson:** the `Redacted()` slog-attr discipline only protects structured log sinks — every `Error()` method on a credential-adjacent type needs an independent length cap, because callers that `fmt.Errorf("…: %w", err)` and then log the result bypass slog entirely.

- **`err:0b188cab:addon-helpers-lasterr-asymmetric`** — done 2026-06-03 — PR #754, merge commit `34317d7`. Tier K suggestion (wrapping). `RetryDefault` (`internal/addon/helpers.go:29-45`) returned the `wait.ExponentialBackoffWithContext` result verbatim — on exhaustion callers saw `wait.ErrWaitTimeout` instead of the original `fn()` failure. Mirrored the canonical pattern in `internal/download/retry.go::retryDownload`: capture `lastErr` inside the condition closure, return it on backoff exhaustion when not a ctx error. Updated `Test_RetryDefault_AllFailures` to assert sentinel propagation via `errors.Is`; dropped the now-unused `wait` import. **Postmortem lesson:** symmetric helpers in the same repo must have symmetric error surfaces — when two retry primitives diverge on which error they return at exhaustion, callers cannot share retry-classification logic and the divergence multiplies as new sites are added.

- **`err:d9f7733e:debug-bundle-stringified-errors`** — done 2026-06-03 — PR #755, merge commit `0238e8d`. Tier K suggestion (redaction-in-error — seam→audit-observability). Six call sites in `internal/cli/debug_bundle.go` (bundleConfig/bundleLogFile/bundleTerraformState/bundleDoctor×2/bundleSystemMeta) stringified errors directly into manifestEntry.Message via `err.Error()`, dropping the `Redacted() any` dispatch that `logutil.redactAny` performs. The debug-bundle is operator-shared, so a future credential-carrying error type would leak through every bundle. Added a file-local `safeMessage(err)` helper that type-asserts `interface{ Redacted() any }` before `Error()` (mirrors `logutil.redactAny`'s exact dispatch); replaced all six sites. Added `TestSafeMessageRedacted` covering redactable + plain + nil paths. **Postmortem lesson:** `slog` is not the only sink for errors — wherever an error reaches a structured-output path that operators forward upstream (debug bundles, support payloads, exit messages), the `Redacted() any` dispatch must be replicated; a file-local 8-line helper is cheaper than a centralized utility that everyone forgets to import.

- **`err:aa84670c:root-signalexitcode-invariant-test`** — done 2026-06-03 — PR #756, merge commit `dd8690f`. Tier K suggestion (cancellation-identity — seam→audit-concurrency). `TestUnwrapChainIntact` (`internal/errtypes/errtypes_test.go:42-57`) only probed ClusterError × DeadlineExceeded; the other four errtypes and context.Canceled were uncovered. A future errtype wrapping ctx via `fmt.Errorf("…: %v", ctx.Err())` (no `%w`) would silently break SIGINT→130 mapping in `signalExitCode`. Rewrote the test table-driven: five errtypes × {Canceled, DeadlineExceeded, unrelated} = 15 sub-cases, with each errtype's `wrap` closure preserved as a constructor; original `os.ErrNotExist` row kept as an additional positive case. Test-only change. **Postmortem lesson:** invariant tests for cross-package contracts (here: errtype → signalExitCode → exit code) must enumerate the FULL matrix because the failure mode is silent — no compile error, no panic, just a wrong exit code that scripts depending on `$?` for retry decisions will miss.

- **`err:fde34e0c:k8s-subcommand-load-bearing-comment`** — done 2026-06-03 — PR #757, merge commit `20d0ea0`. Tier K suggestion (redaction-in-error — seam→audit-observability). `cluster.subcommand()` (`internal/cluster/k8s.go:126-138`) returns args[0] only — preventing credential-bearing argv (e.g. `--from-literal=password=…`) from reaching `executor.ExitError.Command`. The contract was enforced by reviewer discipline only; no test would catch a future change that passed full argv. Added `TestExitErrorCommandNoArgvLeak` in `internal/errtypes/errtypes_credleak_test.go` with three sub-cases: safe pattern (cli+subcommand) → no leak; adversarial canary (full argv with credential) → leak IS present (locks the assumption that ExitError does not self-redact); NewExitError binary-only → no leak. Canary inverted assertion forces a doc update if ExitError ever gains Command redaction. **Postmortem lesson:** "we depend on a downstream guard" deserves a canary test that confirms the downstream is NOT redundantly redacting — when the downstream silently adds protection, the upstream guard becomes dead code, and the canary forces a coordinated change.

- **`err:40d315ad:addon-flux-error-as-string`** — done 2026-06-03 — PR #758, merge commit `18988d0`. Tier K suggestion (string-sniffing — seam→audit-api-design). `Addon.ValidateSettings` (`internal/addon/addon.go:54`) returns `[]string` because the wizard UI needs user-display strings; the audit flagged the risk that a future caller might use the result for orchestrator-side exit-code branching, dropping typed-error chains. Doc-only fix: added a 3-line comment naming the wizard-UI contract, forbidding exit-code branching, and pointing reviewers to `cfg.Validate()`. **Postmortem lesson:** when an interface's return type forces information loss (here: typed errors → strings), the type's doc must name the legitimate consumer AND forbid the illegitimate ones — otherwise the next reader assumes any use is fair game and the typed-error chain quietly breaks.

- **`api:bbc23e42:pkg-progress-reporter-location`** — done 2026-06-03 — PR #759, merge commit `0ea335d`. Tier K suggestion (package-boundary). `logutil.ProgressReporter` type (`internal/logutil/logutil.go:23-29`) lives in logutil but the real implementation (`tui.StartSpinner`) lives in internal/tui; the placement looks structurally odd. Verified the move is unsafe: `internal/tui/logger.go` already imports `internal/logutil` for RedactHandler, so moving ProgressReporter to tui would force phase/infrastructure packages that store reporter fields to import tui, completing a cycle. Added a 6-line doc paragraph naming the exact cycle (`tui → logutil` exists; placing the type in tui would force phase/infrastructure → tui → logutil → tui). Doc-only fix. **Postmortem lesson:** every "this type seems to live in the wrong package" smell deserves an import-graph check before action — the unnatural placement is almost always a cycle-break compromise, and the doc must name the cycle so the next refactor doesn't re-create the cycle by "fixing" the smell.

- **`api:21dc1103:opt-name-clash-extract-option`** — done 2026-06-03 — PR #760, merge commit `bc488a4`. Tier K minor (option-consistency). `internal/download` exposed two option surfaces with asymmetric naming: `Option`/`WithChecksum` for Fetch vs `ExtractOption`/`WithExtractChecksum` for Extract. Renamed `download.Option` → `download.FetchOption` and `download.WithChecksum` → `download.WithFetchChecksum`; updated all 3 external call sites (coreos.go, release_extract.go, tools.go) and 1 test call site. Other With* options on Fetch (WithDescription/WithTimeout/WithOverwrite/WithLogger) kept their unqualified names since they have no Extract-side collision. **Postmortem lesson:** when two adjacent functions in the same package each expose a With* option set, the type names must carry the qualifier (FetchOption vs ExtractOption) AND any colliding helper names — partial qualification (only the type) reads as a half-finished rename.

- **`ux:8d8faa80:help-no-example`** — done 2026-06-03 — PR #761, merge commit `00cb759`. Tier K suggestion (help-text). `completionCmd` (`internal/cli/completion.go:14-25`) had bash/zsh/fish shell snippets buried in `Long`; cobra renders Example in a dedicated section so the snippets were less discoverable than they should be. Moved the per-shell blocks from Long into Example with two-space indentation matching the deploy.go/destroy.go convention; Long shrunk to a single descriptive sentence. Required a follow-up `make docs` regeneration commit after the docs-go CI job flagged drift in `docs/cli/okdctl_completion.md`. **Postmortem lesson:** every cobra Long/Example/Short edit must be followed by `make docs` in the same commit — the docs-go CI job diff-checks `docs/cli/` against cobra metadata, and the docs-go vs feature-commit split produces a transient CI red on first push that costs one extra round-trip.

- **`ux:4583b75b:output-flag-inconsistent`** — done 2026-06-03 — PR #762, merge commit `abf0bb0`. Tier K suggestion (flag-conventions). `configShowCmd` (`internal/cli/config.go:25-30`) emitted YAML unconditionally with no `--output` flag, while every other read-only command (status, describe, releases list/show, doctor, addon list/verify) accepts `--output=text|json`. Added the flag with `-o` shorthand (per CLAUDE.md allowlist), reused `validateFormat` and the `outputText`/`outputJSON` constants; default preserves existing YAML behavior, `--output=json` emits the same `redactConfig(cfg)` value as indented JSON. Added `TestRunConfigShow_JSONOutput` asserting TokenID redaction. Appended a `config show --output=json` section to `docs/cli/json-schema.md`. Required `make docs` follow-up commit to refresh `docs/cli/okdctl_config_show.md`. **Postmortem lesson:** when every sibling command in a parent group accepts a flag, the missing-from-one command IS a UX bug — even if the missing case is preserved in the default behavior, scripts piping through `jq` cannot opt in without the flag.

- **`ux:e7db1220:flag-completion-inconsistent`** — done 2026-06-03 — PR #763, merge commit `43100b0`. Tier K suggestion (flag-conventions). `releases list --channel` (`internal/cli/releases.go:75-76`) accepted one of two string enum values (stable, all) without registering a flag-completion function; sibling commands (destroyCmd's `--only`) DID register completions, so shell tab-completion was inconsistent across the surface. Added `RegisterFlagCompletionFunc("channel", …)` returning `[]string{channelStable, channelAll}` with `ShellCompDirectiveNoFileComp`, mirroring the destroy.go pattern. Two-line change. **Postmortem lesson:** any cobra flag that accepts a small string enum should register a completion func at declaration time — the cost is two lines, the benefit is a discoverable surface that doesn't surprise users by silently accepting invalid values without tab-complete feedback.

- **`ux:0d318f5c:flag-no-default-in-help`** — done 2026-06-03 — PR #764, merge commit `7d36193`. Tier K suggestion (flag-conventions). `root.go:302-303` declared `--log-format` with default "text" then immediately overwrote `Lookup(flagLogFormat).DefValue = ""` to hide the printed default in --help (because the auto-switch to JSON when stderr is piped contradicts a static "text" default). The override looked like a stylistic tweak; a future contributor could delete it without realizing the help-text prose would then contradict the printed default. Added a 3-line WHY comment pinning the override as load-bearing for the TTY-vs-pipe contract and warning a future contributor not to remove it without also updating the flag's Usage string. **Postmortem lesson:** load-bearing one-liners with no obvious purpose need a comment naming their invariant — the cost of the comment is far less than the cost of a regression where the default reappears and contradicts the prose.

- **`obs:366b3f2d:level-error-not-user-visible`** — done 2026-06-03 — PR #765, merge commit `cfe52ee`. Tier K suggestion (level-discipline). Orchestrator (`internal/distribution/orchestrator.go:184-188`) logged a fatal step at Error AND returned the error to the caller, which the cli layer (`cli/root.go::execute`) re-logged at Error via `tui.Error("command failed", …)` — producing two Error lines for one event in the user-facing stream. Collapsed the if-fatal-Error / else-Warn branch into a single Warn carrying `fatal=step.IsFatal()` as a structured attr; the cli layer remains the only Error site on command failure. Added a 1-line WHY comment naming the cross-package coupling so a future contributor doesn't normalize back to Error. **Postmortem lesson:** log-level discipline is a cross-package contract — a single layer that promotes-to-Error in isolation creates double-log when the next layer also promotes; the fix is naming which layer owns user-facing Error and demoting the inner layers to Warn with the rationale in-code.

- **`smell:820b96c9:stringly-typed-enum`** — done 2026-06-03 — PR #766, merge commit `ea9a541`. Tier K suggestion (magic-strings). secretstore provider selection (`internal/addon/catalog/secretstore/providers.go:27-52`) routed through bare-string constants (`providerOnepassword`/`providerVault`/`providerBitwarden`) keyed in `map[string]provider` with no typed enum; `resolveProvider` returned `(provider, string)` and `Settings.Provider` was `string`. Mirrored the `cleanup.Kind` / `bundleStatus` / `VMState` / `NodeRole` pattern: introduced `type ProviderKind string` with exported `ProviderOnepassword/Vault/Bitwarden` constants; keyed the providers map on `ProviderKind`; `resolveProvider` returns `(provider, ProviderKind)`; `Settings.Provider` is now typed. YAML wire-format compatibility preserved (underlying type is string). All call sites (Install, Verify, Uninstall, installPrereqCheck, DefaultSettings, WizardFields) updated; the string cast lives at the WizardField.Group boundary and the DefaultSettings map value. **Postmortem lesson:** the canonical typed-string-enum pattern in this repo is `type X string + const ( A X = "a"; B X = "b" )` keyed on the type — adopting it costs ~3 sites per addon and eliminates an entire class of typo bugs at the cost of a couple of `string()` conversions at the wizard's display boundary.

- **`smell:6424733c:abstraction-single-caller`** — done 2026-06-03 — PR #767, merge commit `5f9ffa0`. Tier K suggestion (helper-package-no-value). `tuiReporter` (`internal/cli/helpers.go:191-197`) was a 6-LOC factory whose doc claimed it existed so domain code could capture ctx without taking a tui dependency — but the file already imports internal/tui at L25, so the dependency-isolation claim was false. Grep confirmed exactly one caller. Inlined the closure at the call site (`provOpts = append(provOpts, okd.WithProgressReporter(func(desc string) func() { return tui.StartSpinner(ctx, desc) }))`); removed the misleading doc comment and the named helper. **Postmortem lesson:** a single-caller factory whose stated rationale is false reads as "I'd rather not look at this 4-line closure inline" — the inline form is shorter AND truthful, and the named helper carries the cost of the misleading doc until someone audits it.

- **`smell:efff8856:enum-ad-hoc`** — done 2026-06-03 — PR #768, merge commit `e49190e`. Tier K suggestion (magic-strings). Flux addon's `DefaultSettings()` (`internal/addon/catalog/flux/flux.go:273`) spelled "flux" as a literal; `internal/tui/wizard/steps/defaults.go:71` independently declared `DefaultGitOpsProvider = "flux"`. Two unconnected sites spelling the same enum value invites silent drift. Added `const ProviderID = "flux"` in `flux/flux.go` (the addon owns its identity); `DefaultSettings` references it; `wizard/steps/defaults.go::DefaultGitOpsProvider = flux.ProviderID` after adding the import. Verified no import cycle (wizard already imports flux for addon catalog). **Postmortem lesson:** when two packages independently encode the same identity string, the canonical fix is "the package that owns the identity exports a constant; the other imports it" — moving the constant to a neutral location is overengineering until a third consumer arrives.

- **`ux:4583b75b:json-quiet-inconsistent`** — done 2026-06-11 — PR #797, merge commit `2c74d88`. Tier L minor. `runConfigShow` (internal/cli/config.go:63-66) validated `--output` but was the only JSON-capable command skipping `quietForJSON`, so `config show -o json 2>&1 | jq` saw info chatter every sibling suppresses. One-line fix matching runStatus/runReleasesList/runDoctor/versionCmd. **Postmortem lesson:** when a convention is enforced per-command rather than in shared plumbing, every new JSON-capable command is a fresh chance to forget it — the validateFormat helper would be a better home for the quiet call.

- **`err:ddf885f4:vocab-ad-hoc-synonym`** / **`api:ddf885f4:zero-value-unusable`** — done 2026-06-11 — PR #798, merge commit `66ef6c5`. Tier L minor ×2. InstallOne returned the Resolve() dependency error bare (exit 1) while InstallAll wrapped the identical failure in ConfigError (exit 2); and NewManager's doc claimed phase.NewBasePhase-style nil-safety but only defaulted the logger — a Manager without WithExecutor carried a nil executor into the first addon Install. Wrapped InstallOne identically and defaulted `m.exec` post-options (which also covers `WithExecutor(nil)`); both behaviors test-locked. **Postmortem lesson:** a doc comment that claims parity with a canonical constructor is a contract — audit each defaulted field, not just the first one.

- **`mod:696d6b0e:use-strings-splitseq`** — done 2026-06-11 — PR #799, merge commit `23ffefa`. Tier L minor. Two range-over-strings.Split loops in phase/iso_cleanup.go materialized slices for iteration-only use; moved to strings.SplitSeq (the established repo idiom). The `return true` inside one loop body works unchanged under range-over-func. **Postmortem lesson:** none beyond the idiom — pure mechanical modernization.

- **`mod:88fd3050:use-omitzero`** — done 2026-06-11 — PR #800, merge commit `395dd40`. Tier L minor. Four non-pointer struct fields (Disks, Bootstrap, StaticIP, Bastion) carried `json:",omitempty"` tags that encoding/json silently ignores on struct values — the tag was a no-op since the fields were added. `omitzero` (Go 1.24) implements the stated intent; loading is key-driven and unaffected. Reviewer verified round-trip losslessness empirically against pinned sigs.k8s.io/yaml v1.6.0. **Postmortem lesson:** `omitempty` on a non-pointer struct field is always a bug (intent without effect); grep for the pattern when bumping the language baseline past a feature that fixes it.

- **`obs:ed55ee90:key-inconsistent-casing`** — done 2026-06-11 — PR #801, merge commit `c4b5690`. Tier L minor. cleanup/summary.go's clean and dirty branches logged the same metric under different keys (`files` vs `count`); unified to one `remaining` key per metric across all three pairs. **Postmortem lesson:** branch-paired log lines should share an attr-building helper, or the keys drift the moment one branch is edited.

- **`sub:06f00bcb:exit-code-ignored`** / **`tst:06f00bcb:cred-perms-policy-untested`** — done 2026-06-11 — PR #802, merge commit `c5df89f`. Tier L minor + major. `a2enmod ssl`/`a2enconf` ran via Exec.Run, which returns err=nil on non-zero exits, so the intended warn never fired; moved to RunChecked (still non-fatal). The web-root credential invariants — ignition files 0640 inside 0750 ignition/, kubeconfig/kubeadmin-password never copied to the DocumentRoot — had zero tests; now table-locked including the absent-file skip and warn-only chown. This PR also hit the x/crypto v0.53.0 runner-death on CI and went green after rebasing onto the #824 pin. **Postmortem lesson:** `Run` vs `RunChecked` is the recurring okdctl footgun — any call site that branches on failure must use the Checked variant, because the transport-error check reads like a failure check and isn't.

- **`mod:366b3f2d:use-slices-clone`** — done 2026-06-11 — PR #803, merge commit `98de803`. Tier K minor. Three make+copy defensive copies became slices.Clone; the fourth site (executor.SnapshotEnv) was deliberately kept because its test pins a non-nil empty snapshot and the nil/non-nil distinction is load-bearing for cmd.Env inherit semantics. **Postmortem lesson:** slices.Clone(nil) returns nil where make+copy returned non-nil empty — the conversion is only mechanical after auditing every consumer for that distinction, and an intentional skip deserves a commit-body note.

- **`smell:91abd90c:magic-number-duplicated`** — done 2026-06-11 — PR #804, merge commit `88aab33`. Tier L suggestion. The VMID base 6000 lived as bare literals in config/defaults.go and proxmox.go's enumeration-probe fallback; drift would have silently inverted the probe's log-suppression heuristic. Exported `config.DefaultVMIDBase` referenced from both. **Postmortem lesson:** a duplicated literal is only worth a constant when the two sites must agree for a behavior to hold — this one qualified because the probe compares against the seeded default.

- **`err:4c092fce:ctx-err-unwrapped`** — done 2026-06-11 — PR #805, merge commit `b2b4381`. Tier L major. terraform run()/Output() built ExecError directly, hiding context.Canceled from the chain, so Ctrl-C during apply/destroy/output mapped to exit 2/4 instead of 130/143. Replaced with executor.NewExitError, the canonical ctx-aware constructor. Reviewer flagged the same latent pattern in setup/release_extract.go:134 (out of scope, left for a future item). **Postmortem lesson:** once a package exports a ctx-aware error constructor, direct struct construction anywhere else is a bug class — worth a one-off grep when touching the error type.

- **`err:7b2829bb:ctx-err-unwrapped`** — done 2026-06-11 — PR #806, merge commit `18a039e`. Tier L major. RunInteractive returned the raw process error after SIGINT soft-cancel; now returns ctx.Err() when set (NewExitError's precedence), fixing dry-run plan (exit 2→130) and scp ISO upload (3→130). Round-1 review caught the completion debug log being skipped on the cancel path — the duration field is the WaitDelay-SIGKILL diagnostic, so the log now precedes the early return. **Postmortem lesson:** early-return error paths silently drop end-of-operation telemetry; reorder the log above the return rather than duplicating it per branch.

- **`err:d7ce9d16:dns-bare-fmt-errorf-not-classified`** — done 2026-06-11 — PR #807, merge commit `c00332a`. Tier K minor. Eight validation-shaped fmt.Errorf returns in dns.go mapped to exit 4 only via classifyStepErr's ClusterError fallback; now typed ConfigError (exit 2). Operational failures untouched. Also hit the x/crypto runner-death on CI; green after rebasing onto the #824 pin. **Postmortem lesson:** the classifyStepErr fallback masks unclassified errors well enough that bare fmt.Errorf survives review — the audit grep (`fmt.Errorf` in validation paths) is the only reliable detector.

- **`obs:41a9d4eb:log-leaks-cred`** — done 2026-06-11 — PR #808, merge commit `9240cd7`. Tier L minor. RedactableStderr.Redacted() bounded length but never scrubbed content, so credentials in short (≤400-byte) subprocess stderr reached the sink verbatim. Added a fragment-keyed scrub (password/token/secret/api_key/authorization, Bearer-aware) that runs BEFORE truncation so a window cut can never split a key from its value. Two review rounds: round 1 found truncation-before-scrub; round 2 found JSON-quoted keys and spaced quoted values bypassing the regex — both closed with regression cases. **Postmortem lesson:** redaction regexes must be tested against the diagnostic formats the subprocesses actually emit (Proxmox REST JSON, terraform diagnostics), not just shell-style key=value — quoted shapes are the primary real-world leak path.

- **`err:15ba17da:nonfatal-failures-exit-zero`** — done 2026-06-11 — PR #809, merge commit `d975c3c`. Tier L major. Every destroy step was NonFatal and the summary warn-logged failures then returned nil, so a failed terraform destroy printed "cluster destroyed" and exited 0 with live VMs standing. destroyTracker now retains errors and the summary step (flipped to fatal, mirroring cleanupSummaryStep) returns ClusterError{errors.Join}. Post-merge CI surfaced a funlen overflow in destroySteps — and three comment-trim attempts changed nothing because funlen defaults to ignore-comments: the fix was merging three tracker reads into one multi-assign. **Postmortem lesson:** funlen counts code lines only; comment surgery is never the remedy. And darwin `make lint` had passed locally — the lint gate must run in the same config CI uses before push (existing goos-lint memory now extends to funlen edges).

- **`sec:e3782ee7:symlink-escape`** / **`tst:e3782ee7:canonical-helper-untested`** — done 2026-06-11 — PR #810, merge commit `10ca150`. Tier L major + blocker. CopyFileMode — which writes kubeconfigs and ignition as root under the sudo re-exec — opened dst with O_CREATE|O_TRUNC plus a path-based os.Chmod, so a pre-planted symlink redirected both the write and the chmod. Added the AtomicWrite-style Lstat refusal + O_NOFOLLOW, and moved the perms tighten to fchmod on the open fd. Plan originally dropped the tighten; an existing test caught the contract and the fchmod form preserved it symlink-safely. Symlink-refusal regression test added. **Postmortem lesson:** every root-privileged write helper needs the same three-part shape (Lstat refuse / O_NOFOLLOW open / fd-based chmod) — partial adoption leaves the chmod as the remaining link-following hole.

- **`state:368b892b:stale-bootstrap-sentinel-poisons-redeploy`** — done 2026-06-11 — PR #811, merge commit `1345cd6`. Tier L major. postinstall wrote bootstrap-state.auto.tfvars.json (bootstrap_enabled=false) which cleanup never removed and setup never cleared; terraform loads *.auto.tfvars.json AFTER terraform.tfvars, so a redeploy created no bootstrap VM and WaitForBootstrap timed out after 30 minutes with no diagnostic. Sentinel added to cleanup's removal list and cleared by GenerateTerraformVars on re-render; filename now a shared phase constant. **Postmortem lesson:** any file one phase writes to influence terraform must appear in cleanup's manifest the same day it's introduced — auto-loaded override files are invisible until they bite, because the regenerated tfvars LOOKS correct.

- **`state:62cb8a95:corrupt-state-silent-destroy-noop`** — done 2026-06-11 — PR #812, merge commit `6aeb598`. Tier L major. HasState returned false for an unparseable tfstate and destroy treated false as already-destroyed, returning success while orphaned VMs kept running; Full cleanup then deleted the recovery scaffolding. StateStatus() now distinguishes missing/empty/populated/corrupt; destroy errors on corrupt naming the newest .bak snapshot with restore guidance. HasState kept as a shim so cleanup's post-destroy semantics stayed put. **Postmortem lesson:** a boolean "has state?" accessor conflates four conditions with different correct behaviors; classify at the source and let each caller pick, rather than patching call sites.

- **`sec:aa0f50f5:input-storage-name-unvalidated`** / **`tst:aa0f50f5:trust-boundary-untested`** — done 2026-06-11 — PR #813, merge commit `8e36c9d`. Tier L major ×2. ISOStorage/DataStorage/Bridge/CPUType flowed verbatim into terraform.tfvars HCL string literals — a double quote broke out of the literal and `${}` interpolated. Three review rounds earned their keep: round 1 found the new validators unenforced on ANY deploy path (welcome-mode deployed unvalidated; saveConfig warn-and-proceeded) → hard ScopeProvider gate in runFullDeployment; round 2 found validateProvider no-ops on a bogus provider.type, so a hand-edited type string bypassed the gate with the malicious block intact → gate widened to Required|Enums|Provider — and found dotted PVE storage IDs being over-rejected → dedicated storage pattern. Broad validator tables landed; config floor 12→45. **Postmortem lesson:** adding a validator is the easy half; the audit question is "which code path REFUSES on failure" — and scope-gated validation has a second-order trap where the scope's own validators no-op on a precondition (provider type) that a different scope owns.

- **`state:08c49fc4:update-ingress-workdir-mispass`** — done 2026-06-11 — PR #814, merge commit `67066f7`. Tier L major. runUpdateIngress passed WorkDir=projectRoot where the option means the okd-install dir; the kubeconfig-CA pre-flight then looked at <projectRoot>/cluster-config (never exists), so every haproxy-removal cutover failed AFTER the DNS swap and rolled back — the feature could never succeed from the CLI. Fix: CLI omits WorkDir; defaulting extracted into resolveIngressWorkDir with a unit test, plus an AST check failing the suite if the CLI literal ever sets WorkDir again. Round-1 review rejected the first regression test for exercising RemoveHAProxy with hand-built paths — reintroducing the bug stayed green. **Postmortem lesson:** a regression test must execute the code that produced the bad value, not re-assert downstream behavior with fixture inputs; when the production flow is too heavy to run, extract the decision into a pure function and anchor the call site mechanically.

- **`tst:98723e5d:cred-install-path-untested`** — done 2026-06-11 — PR #815, merge commit `b619669`. Tier L major. SetupClusterAccess installs the cluster-admin kubeconfig into ~/.kube/config as root (0600 copy, timestamped backup, chown-back); only the .bashrc helper had tests. Added a home-dir seam (repo's established statFn pattern) and locked fresh-install perms, backup-on-overwrite byte fidelity, and ctx-cancel leaving dest untouched. **Postmortem lesson:** credential-install paths hide behind sudo-only execution and stay untested by default; a one-var seam makes them cheap to cover without touching behavior.

- **`tst:2f70d7df:trust-boundary-untested`** — done 2026-06-11 — PR #816, merge commit `650d957`. Tier L major. BuildIgnitionURLForNode enforces the RFC1918/loopback/link-local gate that keeps pull-secret-bearing ignition URLs off public interfaces — zero tests, no test file in kargs.go at all. Accept/reject tables landed plus golden kargs strings pinning the `ip=...:none` syntax. **Postmortem lesson:** the highest-value test targets are pure functions guarding security boundaries — table tests cost minutes and the invariant survives refactors.

- **`tst:b5a79fda:trust-boundary-untested`** — done 2026-06-11 — PR #817, merge commit `46c213f`. Tier L minor. readDeployState/announceDeployState (the destroy-entry marker steering cleanup-vs-destroy advice) gained tables: missing file, corrupt JSON, v1 round-trip, unknown-schema ignore, cluster-mismatch ignore. The seven-day stale-age hint stays unasserted for want of a clock seam. **Postmortem lesson:** schema-versioned marker files need their forward-compat ignore path tested most of all — it's the branch that only fires a release later.

- **`tst:48688e63:cred-zeroize-untested`** / **`tst:48688e63:destructive-happy-untested`** — done 2026-06-11 — PR #818, merge commit `232c651`. Tier L major ×2. Proxmox Provider.ZeroizeEnv had no test (secret blanking, slice nil, idempotence — now a superset of the executor sibling's table), and the deploy path's IP arithmetic existed twice (retrieveProvisionResult and setup.BuildNodeList) with zero tests on either. Cross-locked both against identical configs and hard-coded expected sequences so divergence fails exactly one test; initIsRetryable covered; proxmox floor set at 20 (measured 22.7%). A planner fixture bug ("range overflows cidr" case that didn't actually overflow) was caught at apply-verify time. **Postmortem lesson:** duplicated arithmetic across packages is best tested by one shared table asserting both implementations agree — unit tests per copy can both pass while the copies drift.

- **`sub:1e8ffb91:ring-truncated-stdout-parse`** / **`sub:6b533f2d:…`** / **`sub:632c9087:…`** / **`sub:d31d1b9d:…`** — done 2026-06-11 — PR #819, merge commit `48dc087`. Tier L blocker + major ×3. Executor.Run's 200-line ring tail fed machine-parsed `oc … -o json` in four places: VerifyClusterHealth failed on every live cluster, PendingCSRs truncated exactly when approval mattered, discoverIngressControllers' rollback payload could be a truncated head, and `status` swallowed jsonErr reporting empty nodes. Added RunOutput/RunOutputChecked (4 MiB LimitReader cap, Result.Truncated) and BasePhase.OcOutputFull; migrated all four sites with per-site failure semantics (CSR/ingress hard-fail on truncation, status warns). Round-1 review reproduced a hard deadlock — nothing drained the pipe after the limited read, so a child writing more than the kernel pipe buffer past the cap blocked in write and Wait never returned; fixed by draining to discard before Wait, locked by a 2 MiB-vs-10-byte test (the original test exceeded the cap by 90 bytes and passed by fitting in the pipe buffer). Also fixed: ring Truncated fired at exactly 200 lines with nothing dropped; RunStreamed never set it. Merge required a manual rebase: its executor_test.go conflicted with #806's test added at the same append point. **Postmortem lesson:** capped pipe reads MUST drain the remainder before Wait — and truncation tests must exceed the cap by more than the OS pipe buffer, or they validate nothing. A reviewer who writes probe tests catches what diff-reading cannot.

- **`state:c287d5c0:prepare-wipes-live-cluster-artifacts`** — done 2026-06-11 — PR #820, merge commit `175c5ef`. Tier L blocker. Prepare unconditionally wiped okd-install before every deploy — re-running deploy against a healthy cluster deleted cluster-config/auth (the only plaintext kubeadmin-password) with no confirmation. The guard went through three review rounds: round 1 — keying on auth-presence broke the documented re-run-to-resume flow (setup itself writes auth mid-run) and the rejection corrupted the deploy-state marker; round 2 — a refusal-planted marker made the operator's immediate retry bypass the guard (the two-invocation credential wipe), and the resume wipe deletes the marker so a mid-setup kill left auth-present+marker-absent guard-blocked. Final design: tfstate-with-resources is the sole live-cluster trigger (auth alone is mid-setup debris), the marker gates ResumeInProgress, and the CLI probes GuardPrepare BEFORE any marker write; --fresh (long-form) is the escape hatch; pre-deploy cleanup failure is fatal; guard matrix table-tested; refusal is ConfigError per the precondition-refusal precedent. funlen forced extracting runGuardedPrepare, which usefully co-locates the probe-before-write ordering invariant. **Postmortem lesson:** guards over destructive operations interact with every lifecycle the guarded artifact participates in — the marker that distinguishes "interrupted" from "completed" is itself deleted by the operation being guarded, and a rejection that mutates state creates its own bypass. Walk the two-invocation sequence, not just the single run.

- **`ux:073d24ed:concept-named-twice`** / **`ux:073d24ed:yes-flag-divergent-semantics`** / **`ux:073d24ed:exit-code-undefined`** / **`doc:073d24ed:readme-flag-ghost`** — done 2026-06-11 — PR #821, merge commit `a69ba54`. Tier L major ×2 + minor + major. deploy ignored the root --config flag its own Example advertised (now honored; explicit --output-file wins; credentials env derivation in sync); --yes help now states the writes-config-only contract instead of implying it deploys; invalid-config-under---yes, wizard failure, and debug-bundle path refusals now exit 2 via ConfigError (wizard user-cancel still 0, SIGINT still 130); docs/cli regenerated. One review round-trip for an import-ordering gofumpt break. **Postmortem lesson:** a flag advertised in an Example but silently ignored is worse than an error — it trains users into a workflow that does nothing; cobra Examples deserve the same review scrutiny as code.

- **`doc:b3356305:readme-default-drift`** / **`doc:b3356305:readme-behavior-drift`** / **`doc:b3356305:readme-dead-link`** / **`doc:b3356305:readme-flag-ghost`** / **`doc:1013f4e8:docs-behavior-drift`** — done 2026-06-11 — PR #822, merge commit `fe5cc9d`. Tier L major ×5. README said `.env` (real file: okdctl.env — the documented uninstall left the password file on disk); documented plain-HTTP ignition on 8080 where the code serves HTTPS on 443 with coreos-installer-pinned CA (two stale 8080 log strings in apache.go fixed alongside); the curl install URL pointed at a nonexistent main branch in three places (README, in-binary update notice, install.sh header); exit-codes.md described the elevation policy inverted. The readme-flag-ghost item shipped no diff here by design — satisfied by #821, with the merge-order dependency stated in both PRs. Reviewer fact-checked every claim against the code rather than the diff alone. **Postmortem lesson:** docs items deserve code-verifying review, not prose review — the elevation-policy inversion read fluently and only the elevation.go cross-check caught that the original text was backwards.

- **`iac:bc37d034:lockfile-gitignored`** / **`dep:98cc639a:v0x-unregistered-provider`** — done 2026-06-11 — PR #823, merge commit `ad4460c`. Tier L major + suggestion. The root .gitignore suppressed .terraform.lock.hcl repo-wide, so the deployed root module re-resolved provider hashes on every init — the IaC analog of gitignoring go.sum — while the only on-disk lock was untracked cruft pinning the previous provider version. Narrowed the ignore to module locks, committed the production lock (bpg/proxmox 0.109.0, linux_amd64+arm64, generated at terraform 1.14.9 via `providers lock` after an init -backend=false), removed the stale module lock, and registered bpg/proxmox in CLAUDE.md's v0.x registry. **Postmortem lesson:** a gitignore pattern added for module-level dev artifacts silently swallowed the one lock file that mattered; ignore patterns for lock-shaped files should be path-anchored from day one.

- **`ci-testgo-xcrypto-053-runner-death`** — done 2026-06-11 — PR #824, merge commit `8ae883f`. Tier 0 blocker. Since renovate's x/crypto v0.52.0→v0.53.0 bump, `go test ./...` on ubuntu runners died with a runner-agent SIGTERM (exit 143, OOM-kill signature) at the same point; darwin passed, implicating the linux-only test surface (the debug-bundle test re-execs the binary; v0.53.0's ssh package grew ~1000 lines). Pinned back to v0.52.0 with a roadmap-ID-linked comment and a renovate packageRule ignore until upstream is understood. Acceptance verified post-merge: the two PR branches that re-ran test-go on the pinned base (#802, #807) went from deterministic exit-143 to green, confirming the pin as the proximate fix. **Postmortem lesson:** when CI dies at a consistent point only on the memory-constrained platform after a dependency bump, bisecting the bump beats instrumenting the tests — and the revert-with-tripwire (renovate ignore + linked comment) keeps the workaround from fossilizing.

- **`tst:0139cb3f:trust-boundary-untested`** — done 2026-06-11 — PR #825, merge commit `d1484358a`. major (audit-tests). Added paths_test.go locking ResolveBinDir/PreflightBinDir env>config>default precedence, the SUDO_USER tilde-expansion seam, and the documented (un-rejected) `..`-traversal Clean result at phase/paths.go:57-111. **Lesson:** a trust-boundary resolver feeding root-owned bindir paths deserves a golden test that turns any future traversal-rejection change into a visible diff.

- **`tst:262af6e4:destructive-partial-untested`** — done 2026-06-11 — PR #826, merge commit `0d7668bfb`. major (audit-tests). Added two PostDestroy=true cases to cleanup_test.go locking the only tfstate-deleting path (cleanup.go:246-259) — non-empty state preserved (DATA LOSS assertion), empty state removed — through the HasState() gate. **Lesson:** the single code path that can delete terraform.tfstate needs both-outcome coverage, not just the preserve case.

- **`mod:632c9087:use-slices-sort`** — done 2026-06-11 — PR #827, merge commit `4e0a511ee`. minor (audit-modernization). Replaced sort.Strings + last-index with slices.Max in restoreHAProxyBackup (update_ingress.go:625) and dropped the sort import. **Lesson:** sorting a slice only to read its maximum is an O(n log n) tell; slices.Max states the intent in O(n).

- **`mod:9d79b841:use-strings-cut`** — done 2026-06-11 — PR #828, merge commit `cd7837cf5`. suggestion (audit-modernization). Converted the SplitN(_,_,2)+len-check in resolveConfiguredISO (coreos.go:60) to strings.Cut. **Lesson:** scope mechanical stdlib sweeps to the evidence file; the sibling sites belong to their own items.

- **`mod:a3577f6c:use-slices-containsfunc`** — done 2026-06-11 — PR #829, merge commit `9fa345c91`. suggestion (audit-modernization). Replaced the SAN membership loop in cert.go:77 with slices.ContainsFunc + a hoisted net.ParseIP, adding a `want != nil` guard that closes the old nil==nil false-match edge. **Lesson:** hoisting a per-iteration parse can also tighten a latent edge case — review the nil semantics when you do.

- **`obs:21dc1103:level-error-not-user-visible`** — done 2026-06-11 — PR #830, merge commit `10431146b`. minor (audit-observability). Demoted the inner "giving up after retries" record in download.go:127 from Error to Warn so one failure no longer double-logs at ERROR (the boundary log at root.go remains). **Lesson:** log-and-return at Error duplicates the boundary's ERROR; the inner record's value is its attrs, not its level.

- **`obs:40d315ad:level-warn-should-info`** — done 2026-06-11 — PR #831, merge commit `1173ad9ca`. minor (audit-observability). Demoted the two flux advice lines (flux.go:124-126) to Info, keeping only the real degradation at Warn. **Lesson:** remediation/advice text logged at Warn inflates alert counts; only the degradation event warrants Warn.

- **`obs:a6e38cc7:key-inconsistent-casing`** — done 2026-06-11 — PR #832, merge commit `c74bc05f0`. minor (audit-observability). Aligned sshpin's observed-fingerprint attr key to "observed_fingerprints" (sshpin.go:81), matching the flux site. **Lesson:** same concept, same key — divergent attr names defeat field-stable log querying.

- **`doc:8f46b665:docs-struct-snippet-drift`** — done 2026-06-11 — PR #833, merge commit `27dcf8846`. minor (audit-documentation). Added the Reporter field to phases.md's BasePhase snippet and corrected the AlreadyDone guidance (BuildSteps panics for ReRunSafeNo steps without it). **Lesson:** doc snippets that present as the full struct must track the real type; the panic message was quoted verbatim from step.go.

- **`doc:024a2c32:docs-schema-drift`** — done 2026-06-11 — PR #834, merge commit `08718bdcb`. minor (audit-documentation). Added ,omitempty to ClusterStatus.Nodes (types.go) so a nil slice no longer emits "nodes":null — honoring json-schema.md's null-never-emitted contract — and documented nodes[].status. **Lesson:** a stability contract is only real if the struct tags enforce it.

- **`doc:73ad30ef:doc-comment-stale`** — done 2026-06-11 — PR #835, merge commit `656ad8a0a`. suggestion (audit-documentation). Repointed WarnOnError's doc (phase/helpers.go:11) from the non-canonical StepBuilder.OnError() to the StepDef.OnError field that all 8 call sites use. **Lesson:** doc comments that name an API steer new code; keep them on the canonical surface.

- **`smell:c19ee328:magic-strings`** — done 2026-06-11 — PR #836, merge commit `ffe712216`. minor (audit-code-smells). Named ocBin/kubectlBin consts beside openshiftInstallBin in setup/phase.go and used them in the setupBaseSteps trio (steps.go:89). **Lesson:** a typo in a bare AlreadyDone-guard literal is a silent step-skip; name the binary trio.

- **`smell:de572c63:magic-path-literal`** — done 2026-06-11 — PR #837, merge commit `24d6676b4`. minor (audit-code-smells). Derived ConfigureSystemResolver's confPath/confDir from the test-injectable resolvedConf var (dnsmasq.go:208) so configure honors the same seam restore already uses. **Lesson:** a write path that ignores the injection var its read path uses lets the two spellings drift apart.

- **`err:d9f7733e:err-formats-cred`** — done 2026-06-11 — PR #838, merge commit `e1a052cd7`. minor (audit-errors). Routed the two raw-stderr bundle Messages (debug_bundle.go:276,317) through logutil.RedactableStderr so the command's "credentials are redacted" promise holds. **Lesson:** any subprocess stderr reaching an operator-shared artifact must pass the same redaction the error types use.

- **`con:ae5b624c:ticker-zero-interval-panic`** — done 2026-06-11 — PR #839, merge commit `ad470389b`. suggestion (audit-concurrency). Guarded MonitorInstallation's CSRApprovalInterval against <=0 before time.NewTicker (monitor.go:117), mirroring system.WaitFor's zero-guard. **Lesson:** an exported plain Options struct invites zero-value callers; guard the panic-prone ticker input rather than trusting the constructor.

- **`sub:e552bb7d:argv-unvalidated-token`** — done 2026-06-11 — PR #840, merge commit `ac8d7c819`. minor (audit-subprocess, CWE-88). Validated the nmcli connection name (shared regex + explicit leading-dash refusal) in netutil/iface.go before it reaches the `nmcli connection modify` argv. **Lesson:** a token parsed from command output and placed in a dash-significant argv slot is an injection vector; the dns sibling already validated the identical data.

- **`sec:19a715fd:symlink-escape`** — done 2026-06-11 — PR #841, merge commit `054ded8c8`. minor (audit-security, TOCTOU). Rewrote readSecret (secretstore.go:256) to Lstat-refuse symlinks + O_NOFOLLOW read with the perm gate on the lstat inode, matching the tree's readNoFollow pattern. **Lesson:** os.Stat+os.ReadFile both follow symlinks; a link planted during a sudo re-exec redirects the read into an arbitrary root-readable file.

- **`con:6424733c:go-no-wait`** — done 2026-06-11 — PR #842, merge commit `24a467e99`. minor (audit-concurrency). Bound the metrics listener synchronously via net.ListenConfig.Listen(ctx) (helpers.go:267) so a bind failure surfaces before the deploy instead of at exit; the ctx form also satisfies noctx. **Lesson:** launching ListenAndServe in a goroutine before logging "listening" makes the log lie and defers the real error for the whole deploy.

- **`tst:c5e5c304:trust-boundary-untested`** — done 2026-06-11 — PR #843, merge commit `59fb7be5b`. minor (audit-tests). Golden-tested buildTerraformVarsData defaulting (worker-disk and bootstrap-resource inheritance) and the format helpers in setup/terraform.go. **Lesson:** pure config→tfvars renderers silently provision wrong-sized infra; lock the defaulting branches with a golden test.

- **`smell:6179837f:linter-config-bug`** — done 2026-06-11 — PR #844, merge commit `53265b1d1`. suggestion (audit-code-smells). Made goconst's ignore-calls:true explicit with a measured rationale — flipping it false surfaces ~50 call-arg false-positives (secretstore 26x, err 20x, …), so adoption would need min-occurrences>=30. **Lesson:** evaluating a lint knob and recording "not green-keeping, here's why" in-config is a valid resolution of a linter-config finding.

- **`dep:2eef5feb:automerge-v0x-minor`** — done 2026-06-11 — PR #845, merge commit `c5cdf2d2e`. minor (audit-dependencies). Added a Renovate packageRule disabling automerge for v0.x minor bumps so go-proxmox / bpg-proxmox breaking-capable minors get a human eyeball. **Lesson:** CLAUDE.md's "v0.x may break on any minor bump" must be enforced by the automerge config, not merely stated in prose.

- **`ux:daf5bee9:stdout-not-cobra-writer`** — done 2026-06-11 — PR #846, merge commit `792714d0e`. suggestion (audit-cli-ux). runKubeconfig now writes via cmd.OutOrStdout() (kubeconfig.go) instead of os.Stdout, restoring SetOut-testability and parity with sibling RunEs. **Lesson:** the one RunE that bypasses the cobra writer is the one you cannot capture in a test.

- **`iac:ef8f2924:count-exceeds-names`** — done 2026-06-11 — PR #847, merge commit `8e8425ab3`. minor (audit-iac). Added count<=names-length validations to master/worker_count (modules/proxmox-okd/variables.tf) so an over-count fails at plan with a clear message instead of a cryptic slice() panic. **Lesson:** when a count indexes a fixed name list, validate the relationship or operators hit "slice end index out of range".

- **`doc:70b3bae2:docs-struct-snippet-drift`** — done 2026-06-11 — PR #848, merge commit `d20f314b6`. suggestion (audit-documentation). Added KVAsDelimitedString to wizard.md's FieldDefinition snippet to match datadriven.go. **Lesson:** a snippet claiming to be the "smallest unit" struct must carry the FieldTypeKeyValue serialization-toggle field.

- **`api:484b40f0:zero-value-unusable`** — done 2026-06-11 — PR #849, merge commit `c294a89a0`. suggestion (audit-api-design). Lazy-init the Recorder maps inside StepFinished under the existing mutex (deploymetrics/metrics.go) so a zero-value Recorder is usable instead of panicking on first step. **Lesson:** a type that satisfies an interface but only works when built via its constructor is a zero-value footgun; guard or document it.

- **`api:297adb3e:opt-inconsistent`** — done 2026-07-11 — PR #852, merge commit `85bd5d324`. suggestion (audit-api-design). Config.Validate(opts ...ValidationOptions) was a variadic-struct hybrid that silently discarded opts[1:] while coexisting with ValidateWithOptions; Validate() now takes no arguments (ScopeAll default) and scoped callers use ValidateWithOptions. **Lesson:** a variadic that accepts a struct and ignores all but the first element is a silent-misuse API; one entry point per calling shape.

- **`doc:26a430ee:doc-comment-stale`** — done 2026-07-11 — PR #853, merge commit `af1651b09`. suggestion (audit-documentation). ensureRoot's policy doc (elevation.go:78-84) illustrated the reject branch with `sudo okdctl wizard` — a command that does not exist in the cobra tree (the wizard runs inside deploy); replaced with a real non-root command. **Lesson:** doc-comment examples rot when commands move; only name invocations that actually parse.

- **`mod:d9f7733e:use-builtins`** — done 2026-07-11 — PR #854, merge commit `b74da774b`. suggestion (audit-modernization). Replaced the four-line assign-then-reassign clamp in debug_bundle.go:361-365 with min(actualSize, maxBundleFileBytes), keeping actualSize for the downstream truncation decision. **Lesson:** pre-Go-1.21 clamp idioms read as branching logic; the min builtin states the intent in one expression.

- **`iac:ef8f2924:commented-out-vars`** — done 2026-07-11 — PR #855, merge commit `29d26bd40`. suggestion (audit-iac-and-shell). Deleted the 14-line commented-out kernel-args variable block (variables.tf:252-265) that the module's resources no longer wired up. **Lesson:** commented-out HCL invites re-enabling an interface that no longer exists; git history is the archive, the roadmap is the tracker.

- **`api:d31d1b9d:pkg-facade-bypass-status`** — done 2026-07-11 — PR #856, merge commit `63a60af7e`. minor (audit-api-design). cli/status.go constructed a phase.BasePhase just to run oc queries from the CLI layer; promoted raw kubectl-query helpers onto cluster.Client and switched status to them, keeping phase as an orchestration-only primitive. **Lesson:** when a CLI command reaches into a phase primitive to shell out, the canonical client is missing a helper — grow the facade instead of bypassing it.

- **`api:fde34e0c:opt-with-env-fallback-side-effect`** — done 2026-07-11 — PR #857, merge commit `8f916235c`. minor (audit-api-design). cluster.WithEnvFallback reads $KUBECONFIG and probes PATH at option-application time, so its effect depends on option order; documented the must-be-applied-last contract at the option and its call sites rather than splitting out a second factory. **Lesson:** an order-sensitive option with two callers can carry a documented ordering contract; record why the structural fix was declined so the next caller sees it.

- **`obs:48688e63:key-inconsistent-casing`** — done 2026-07-11 — PR #858, merge commit `9c932c4c2`. minor (audit-observability). The planned-VM total was keyed "count" in Provision but "vm_count" in the dry-run preview (proxmox.go:220/293); standardised on vm_count so one jq/grep filter sees both sites. **Lesson:** the same measurement under two attr keys silently halves query results; prefer the self-describing key.

- **`state:6424733c:error-path-hint-always-destroy`** — done 2026-07-11 — PR #859, merge commit `3bedfd742`. minor (audit-state-and-recovery). A non-cancel prepare failure printed "run okdctl destroy" although terraform state was empty, sending operators into a sudo/terraform teardown when cleanup was the right verb; mirrored the cancel branch's phase-aware hints (prepare → cleanup, install/configure → destroy) keyed off the deploy-state marker. **Lesson:** remediation hints are part of the destructive-op surface — a wrong verb in an error message causes real, needless teardown.

- **`obs:97cb8adf:span-no-start-end`** — done 2026-07-11 — PR #860, merge commit `4e2784832`. suggestion (audit-observability). WaitFor's waiting/ready messages lacked the `<subsystem>:` prefix every other log site carries and keyed the wait target as the reserved-word-ish "for"; moved the caller's prefix into the message and renamed the attr to target. **Lesson:** a shared logging helper is where conventions break for every caller at once — the helper must apply the prefix contract itself.

- **`tst:0934cf1b:destructive-happy-untested`** — done 2026-07-11 — PR #861, merge commit `acd445ede`. minor (audit-tests). Covered Manager.IsInstalled/Remove/AddRepo (the root dnf/apt-get remove path in platform/packages.go) via fake binaries. One review round: the initial Remove tests could not fail — the fake dnf always exited 0 and recorded nothing — so the fakes now append argv to a log file and the test asserts `remove -y installed-pkg` exactly (uninstalled package must not appear; the all-uninstalled case asserts the tool is never invoked). Passing the log path via env var failed because system.RunCaptured filters subprocess env through executor.DefaultEnvAllowlist, so the path had to be embedded in the fake script. **Lesson:** a fake that records nothing proves nothing — and env-var seams die silently at env allowlists; bake the recording channel into the fake itself.

- **`tst:632c9087:destructive-partial-untested`** — done 2026-07-11 — PR #862, merge commit `26b006fab`. major (audit-tests). Drove convertToLoadBalancer's delete→wait→create→rollback orchestration through a fake oc binary, covering create-failure rollback and rollback-failure double-error paths. One review round: the new rollback error embedded raw stderr, bypassing the logutil.RedactableStderr convention, and attemptRollback carried a stale "errors are logged only" doc — both fixed; the strengthened test asserts via errors.Unwrap rather than err.Error() because ClusterError.Error() deliberately omits the wrapped error per the errtypes redaction invariant. **Lesson:** tests asserting on err.Error() fight the redaction design — unwrap and match the inner error; and every new error return carrying subprocess stderr must route it through the redaction type like its siblings.

- **`smell:073d24ed:stepdef-name-duplication`** — done 2026-07-11 — PR #863, merge commit `696fb6b81`. minor (audit-code-smells). deployDryRunSteps hardcoded all 32 step ID/Name pairs, silently desynchronizing from the StepDefs in setup/install/postinstall; each phase now exports a StepNames table built from the same literals and the dry-run listing ranges the three tables in execution order. **Lesson:** a listing that mirrors authoritative data by re-typing it will drift with no compile or test failure — single-source the strings or add a drift guard.

- **`ux:024a2c32:json-exit-contract-drift`** — done 2026-07-11 — PR #864, merge commit `86cc12238`. minor (audit-cli-ux). json-schema.md claimed exit 0 on JSON success universally, but addon verify exits 4 on probe failure and doctor exits 2 on failing checks; scoped the convention to `status` and listed the exceptions explicitly instead of changing shipped exit behavior. **Lesson:** a blanket contract only one command honors is worse than no contract; fix the doc's scope, never the exit codes scripts already depend on.

- **`smell:15ba17da:magic-label-sentinel`** — done 2026-07-11 — PR #865, merge commit `3b01858ef`. suggestion (audit-code-smells). destroyTracker.terraformFailed gated ISO/firewall cleanup scope on slices.Contains(failures, "terraform destroy") — a free string that had to stay byte-identical to the track() label 23 lines away, where a reword would silently run ISO removal against live VMs; extracted the shared label constant. **Lesson:** when one string is both a display label and a control-flow sentinel, a single constant must own it — or key the logic on the StepID that already exists for exactly this.

- **`state:262af6e4:cleanup-tfvars-proxy-precondition`** — done 2026-07-11 — PR #866, merge commit `a21ca2a98`. minor (audit-state-and-recovery). StepCleanupTerraform's AlreadyDone used terraform.tfvars existence as a proxy for the whole artifact set, so a partial prior cleanup skipped the remaining artifacts permanently; already-done now requires every terraformFilesToRemove entry and the .terraform/ dir to be absent. One review round: CI funlen failed (cleanupSteps hit 124 > 120 lines); fixed by extracting a package-level terraformCleanupDone helper into infra.go next to terraformFilesToRemove. **Lesson:** an AlreadyDone precondition must cover the step's full work product, not its cheapest sentinel — and a multi-file check belongs in a named helper, which funlen will insist on anyway.

- **`api:21dc1103:opt-inconsistent`** — done 2026-07-11 — PR #867, merge commit `4249c230d`. suggestion (audit-api-design). Fetch and Extract functional-option families shared package download with collision-driven asymmetric prefixes (WithFetchChecksum beside bare WithLogger; WithExtractChecksum beside bare WithStripComponents); every Extract option now carries the prefix so each family follows one rule. **Lesson:** in a shared option namespace, prefix by family from the first collision — per-collision patching produces a surface callers must memorize.

- **`ux:e7db1220:help-long-missing`** — done 2026-07-11 — PR #868, merge commit `c893ce5e7`. suggestion (audit-cli-ux). releases list/show, config show, and describe node/addon shipped Short+Example but no Long, leaving --help depth uneven against siblings; added 1-3 sentence Longs (channel filtering and the disk cache, text-mode-emits-YAML note) and regenerated docs/cli. **Lesson:** semantics that live only in flag help (--channel's stable-vs-all) are invisible at the command level; the Long is where operators actually look first.

- **`api:e45c2239:pkg-facade-bypassed`** — done 2026-07-11 — PR #869, merge commit `2a6700d03`. minor (audit-api-design). cmd/okdctl and cli/doctor imported internal/distribution/okd/phase solely for bin-dir path helpers; moved ResolveBinDir/PreflightBinDir and friends into internal/config next to ValidateBinDir (their only validation dependency), decoupling the binary entry point from OKD phase internals. **Lesson:** a helper's home is where its dependencies live — when packages import a heavy package for one generic function, the function is in the wrong package.

- **`state:881d089e:runlock-release-unlink-race`** — done 2026-07-11 — PR #870, merge commit `5323af173`. minor (audit-state-and-recovery). Release closed the flock fd then unlinked the lockfile, reintroducing the classic race where two holders flock different inodes at the same path; the zero-length lockfile now stays in place so flock identity binds one stable inode. One review round: the fix was correct but created a privilege regression — sudo re-exec'd deploy/destroy creates the now-persistent lockfile root-owned 0600, so later non-root --dry-run invocations (which skip elevation but still Acquire) failed EACCES; fixed with a chownToSudoInvoker helper reusing system.ChownToInvokingUser on every root acquisition, which also heals pre-fix root-owned files. **Lesson:** making a temp artifact persistent changes its ownership story — any file a sudo re-exec creates and a non-root path later opens needs an explicit chown-to-invoker, and the heal must cover files created before the fix.

- **`err:40d315ad:vocab-ad-hoc-synonym`** — done 2026-07-11 — PR #871, merge commit `f63dba02c`. minor (audit-errors). flux invalid-settings and secretstore unknown-provider failures returned untyped fmt.Errorf while sibling sites in the same files used errtypes.ConfigError, so the same misconfiguration class exited 4 or 2 depending on which constructor the addon author picked; both now return typed ConfigError and map to exit 2. **Lesson:** the exit-code taxonomy is only as consistent as the least-typed error in the class — when auditing one constructor, grep the file for its untyped siblings.

- **`api:4c092fce:opt-execerror-alias`** — done 2026-07-11 — PR #872, merge commit `e6c5a4a5d`. suggestion (audit-api-design). terraform.ExecError aliases executor.ExitError but nothing said so; documented the errors.As equivalence at the type doc and package doc, keeping the alias since no caller branches on it specifically. **Lesson:** a cross-package type alias is invisible at call sites — without a stated equivalence contract, callers write either-or errors.As branches against what is one type.

- **`api:f99eddfa:opt-options-pointer-asymmetry`** — done 2026-07-11 — PR #873, merge commit `586317a72`. suggestion (audit-api-design). All four phase NewOptions constructors return Options by value while Execute takes *Options, forcing every caller through the address-of dance; documented the intentional value-return rationale at the postinstall canonical site rather than mass-editing four phases. **Lesson:** when both API shapes are defensible, documenting the chosen one closes the finding at a fraction of the churn — the audit's real ask was "explain or align", not "change".

- **`obs:aa84670c:log-leaks-cred`** — done 2026-07-11 — PR #874, merge commit `751b2ac16`. suggestion (audit-observability). The startup audit line joined os.Args into a single string attr, invisible to key-based redaction, so any future secret-bearing flag would log verbatim; argv is now scrubbed for secret-keyed tokens before joining. One review round: the first pass covered --key=value but not pflag's equally-valid space-separated form (--token abc123 leaked); fixed by also scrubbing the following argv entry after a bare secret-named flag unless it starts with '-'. **Lesson:** argv redaction must cover every flag syntax the parser accepts — pflag's space-separated form is the one everyone forgets because = is the documented convention.
