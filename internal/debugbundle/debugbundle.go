...
func Write(ctx context.Context, opts Options) (retErr error) {
	...
	projectRoot, prErr := opts.ProjectRoot()

	sections := collectSections(ctx, addFile, addStream, cfg, cfgErr, projectRoot, prErr, bundleAt, bundleID, opts.LogFile, opts.SkipMustGather)
