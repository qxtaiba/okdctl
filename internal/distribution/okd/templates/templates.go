...
type DNSCustomDomain struct {
	Domain string // e.g., "lab.example.com"
	IP     string // LoadBalancer IP assigned by MetalLB
}

// DNSConfigData is the template binding for dnsmasq bootstrap and production configs.
...
// DefaultKubeVIPImageTag is the kube-vip image tag used when KubeVIPData leaves ImageTag empty.
...
// CompactIngressData is the template binding for the compact-mode ingress controller manifest.
...
// RenderDNSProductionConfig renders the dnsmasq config used once the cluster is live.
func RenderDNSProductionConfig(data *DNSConfigData) (string, error) {
	return renderTemplate("dnsmasq-production.conf.tmpl", data)
}

// KubeVIPRBACManifest is one rendered kube-vip RBAC YAML document with its target filename.
type KubeVIPRBACManifest struct {
	Filename string
	Content  string
}

// RenderKubeVIPRBACManifests renders each kube-vip RBAC manifest as a separate
// document, since openshift-install only processes the first YAML document per file.
...
// RenderChronyConf renders /etc/chrony.conf pointed at data.Server. It steps
// the clock unconditionally (makestep 1.0 -1) instead of slewing, since
// Proxmox pause/resume can jump the clock enough to fail etcd elections and
// cert validity otherwise.
...
// data.Role. It masks FCOS's stock fstrim.timer (no /etc/fstab for `fstrim
// --fstab`, see coreos/fedora-coreos-tracker#468) and ships a replacement
// unit for explicit mountpoints.
