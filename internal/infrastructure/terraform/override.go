...
// DestroyOverrideFileName is the transient override okdctl writes during a
// confirmed destroy to disable the master resource's prevent_destroy; the
...
// DestroyOverridePath returns the transient override file path inside moduleDir.
func DestroyOverridePath(moduleDir string) string {
	return filepath.Join(moduleDir, DestroyOverrideFileName)
}

// WriteDestroyOverride writes (or overwrites a stale copy) the transient
// prevent_destroy override into moduleDir and returns its path. Callers must
// remove it via RemoveDestroyOverride when the destroy finishes, success or failure.
...
// RemoveDestroyOverride removes the transient override from moduleDir; a missing file is success.
...
// staleDestroyOverridePath resolves via the pinned environments/modules
// workspace layout; returns "" when absent.
...
// refuseStaleDestroyOverride fails closed: a stale override outside a destroy
// session would silently defeat prevent_destroy.
...
// WithPreventDestroyHint appends an actionable hint (same mechanics as
// WithLockHint) when err carries Terraform's prevent_destroy diagnostic; the
// override covers only the master resource.
